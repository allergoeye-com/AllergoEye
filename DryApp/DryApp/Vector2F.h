...
struct Vector2F {
	...
	void Set(int ix, int iy) { x = (float)ix; y = (float)iy; }
	Vector2F &operator = (const POINT &p) { x = (float)p.x; y = (float)p.y; return *this;}
	Vector2F &operator = (const CPoint &p) { x = (float)p.x; y = (float)p.y; return *this;}
	Vector2F &operator += (const Vector2F &p)  { x += p.x, y += p.y; return *this; } 
	Vector2F &operator -= (const Vector2F &p) { x -= p.x, y -= p.y; return *this; }
	Vector2F &operator /= (float d)  { x /= d, y /= d; return *this; }
	Vector2F &operator *= (float d)  { x *= d, y *= d; return *this; }
	Vector2F operator * (float d)  {Vector2F v(*this); v *= d; return v; }
	Vector2F operator / (float d)  {Vector2F v(*this); v /= d; return v; }
	Vector2F &WeightedSum(const Vector2F &a, float wa, const Vector2F &b, float wb) { float s = wa + wb; x = (a.x * wa + b.x * wb) / s; y = (a.y * wa + b.y * wb) / s; return *this; }

	Vector2F &operator /= (int d)  { x /= (float)d, y /= (float)d; return *this; }
	Vector2F &operator *= (int d)  { x *= (float)d, y *= (float)d; return *this; }
	Vector2F operator * (int d)  {Vector2F v(x *= (float)d, y * (float)d); return v; }
	Vector2F operator / (int d)  {Vector2F v(x / (float)d, y / (float)d); return v; }
	Vector2F operator / (double d)  {Vector2F v(x / (float)d, y / (float)d); return v; }
	void Shift(float dx, float dy) { x += dx; y += dy; }

	float operator [] (int i) const { return !i ? x : y; }
	float &operator [] (int i) { return !i ? x : y; }
	BOOL operator == (const Vector2F &p) const { return fabs(x - p.x) <= PRECISION_F && fabs(y - p.y) <= PRECISION_F; }
	BOOL operator != (const Vector2F &p) const  { return !operator == (p); }
	...
	float Len() { return (float)sqrt (x*x + y*y); }
	float Norm() { return (float)sqrt (x*x + y*y); }
	float LenQ() { return x*x + y*y; }
	float Modul ()  { return Len(); }
	Vector2F Ort() { float l = Len(); return l ? Vector2F(x/l, y/l) : Vector2F(0, 0); }
	Vector2F Perp() { return Vector2F(-y, x); }
	void Normalize() { float l = Len(); if (l > 0.0) { x /= l; y /= l; } }
...
//
//------------------------------------
inline float operator * (const Vector2F &p, const Vector2F &p1) { return p.x * p1.x + p.y * p1.y; } 
//------------------------------------
//
//------------------------------------
inline Vector2F operator + (const Vector2F &p, const Vector2F &p1) { return Vector2F(p.x + p1.x, p.y + p1.y); } 
//------------------------------------
//
//------------------------------------
inline Vector2F operator - (const Vector2F &p, const Vector2F &p1) { return Vector2F(p.x - p1.x, p.y - p1.y); }
...
//
//------------------------------------
inline Vector2F operator * (float i, const Vector2F &p) { return Vector2F(p.x * i, p.y * i); }
//------------------------------------
//
//------------------------------------
inline Vector2F operator * (int i, const Vector2F &p) { return Vector2F(p.x * (float)i, p.y * (float)i); }
inline Vector2F operator / (int i, const Vector2F &p) { return Vector2F(p.x / (float)i, p.y / (float)i); }
inline Vector2F operator + (int i, const Vector2F &p) { return Vector2F(p.x + (float)i, p.y + (float)i); }
inline Vector2F operator - (int i, const Vector2F &p) { return Vector2F(p.x - (float)i, p.y - (float)i); }
...
//
//------------------------------------
inline float PolarAngle(const Vector2F &p1, const Vector2F &p2,  const Vector2F &p3, const Vector2F &p4)
{
