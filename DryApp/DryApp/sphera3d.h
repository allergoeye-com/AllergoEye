...
struct Sphera3D
{
	...
	Sphera3D(Box3D &b): center (b.CenterPoint()) { radius = ((b.maxPoint - b.minPoint) * 0.5).Len(); radiusQ = radius * radius; }

	Box3D Box();
	BOOL operator > (const Sphera3D &a) const { return fabs(radius - a.radius) > PRECISION ? (radius > a.radius) : (center > a.center); }
	BOOL operator < (const Sphera3D &a) const { return fabs(radius - a.radius) > PRECISION ? (radius < a.radius) : (center < a.center); }
	BOOL operator == (const Sphera3D &a) const { return fabs(radius - a.radius) < PRECISION && center == a.center; }
	BOOL operator != (const Sphera3D &a) const { return !operator == (a); }
	operator Vector3D() const { return center; } 
	void Init(Vector3D &c, double r) { center = c; radius = r; radiusQ = r * r; }
	void Init(Box3D &b) { center = b.CenterPoint(); radius = ((b.maxPoint - b.minPoint) * 0.5).Len(); radiusQ = radius * radius; }
 	void Init(Sphera3D &s) { radius = s.radius; radiusQ = s.radiusQ; center = s.center; }
