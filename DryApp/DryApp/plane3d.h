...
struct Plane3D {
	enum PL { PL_FRONT, PL_FAR, PL_LEFT, PL_RIGHT, PL_TOP, PL_BOTTOM };
	enum {PLANE, FRONT = 1, BACK = 2, BOTH = 4, INTERSECT = 8 };
	Plane3D(){ Init(Vector3D(0, 0, 1), 1); };
	Plane3D(const Plane3D &p) { n = p.n; dist = p.dist; mask = p.mask; }
	Plane3D(const Vector3D v, double d) { Init(v, d); }
    Plane3D(const Vector3D &normal, const Vector3D &v) { Init(normal, v); }
    Plane3D(const Vector3D &p1, const Vector3D &p2, const Vector3D &p3){ Init(p1, p2, p3); }
	...
	bool operator != (const Plane3D &a) const { return !(operator == (a)); }    
	void Init(Plane3D &d) { n = d.n; dist = d.dist; mask = d.mask; }
    ...
    bool IsIntersect(Plane3D &p) { double b = fabs((p.n * n)) - 1; return b < -PRECISION || b > PRECISION; }
	double Distance (Vector3D &v) {return v.x * n.x + v.y * n.y + v.z * n.z + dist; }
    Vector3D Point () { return n * -dist; }
...
struct Plane3F {
	enum PL { PL_FRONT, PL_FAR, PL_LEFT, PL_RIGHT, PL_TOP, PL_BOTTOM };
	enum {PLANE, FRONT = 1, BACK = 2, BOTH = 4, INTERSECT = 8 };
	Plane3F(){ Init(Vector3F(0, 0, 1), 1); };
	Plane3F(const Plane3F &p) { n = p.n; dist = p.dist; mask = p.mask; }
	Plane3F(const Vector3F v, float d) { Init(v, d); }
    Plane3F(const Vector3F &normal, const Vector3F &v) { Init(normal, v); }
    Plane3F(const Vector3F &p1, const Vector3F &p2, const Vector3F &p3){ Init(p1, p2, p3); }
	bool operator < (const Plane3F &a)  const { 
            return (operator != (a)) && (n < a.n || (n == a.n && dist < a.dist)); }
	bool operator > (const Plane3F &a)  const { 
            return (operator != (a)) && (n > a.n || (n == a.n && dist > a.dist)); }       
	bool operator == (const Plane3F &a) const  { 
            float b = (a.n * n) - 1; return b >= 0 && b < PRECISION_F && (dist == a.dist || 
                                            (fabs(n.x) > PRECISION_F && a.n.x/n.x == a.dist/dist) ||
                                            (fabs(n.y) > PRECISION_F && a.n.y/n.y == a.dist/dist) ||
                                            (fabs(n.y) > PRECISION_F && a.n.z/n.z == a.dist/dist)); }
									
	bool operator != (const Plane3F &a) const { return !(operator == (a)); }    
	void Init(Plane3F &d) { n = d.n; dist = d.dist; mask = d.mask; }
    ...
    bool IsIntersect(Plane3F &p) { float b = (float)fabs((p.n * n)) - 1; return b < -PRECISION_F || b > PRECISION_F; }
	float Distance (Vector3F &v) {return v.x * n.x + v.y * n.y + v.z * n.z + dist; }
    Vector3F Point () { return n * -dist; }
    int	Dir(const Vector3F &v);
    void Flip();
