...
struct RBKeyNode {
	...
	bool color;
	MAP_KEY key;
	RBKeyNode () { color = 0; parent = left = right = 0; };
	RBKeyNode (RBKeyNode &r) { operator = (r); };
	RBKeyNode *Reset() { key = 0; color = 0; parent = 0; left = 0; right = 0; return this; }
	RBKeyNode &operator = (RBKeyNode &r) { color = r.color; parent = r.parent; left = r.left; right = r.right; return *this; };
...
struct RBData {
	...
	RBData () {};
	~RBData () {};
	RBData(MAP_KEY _key, MAP_VALUE _value) {key = _key; value = _value; }
	RBData (const RBData &r) { operator = (r); };
	RBData &operator = (const RBData &r) { key = r.key; value = r.value; return *this; };
	bool operator == (RBData &r) const { return key == r.key && value == r.value; };
	void Set(const MAP_KEY &_key, const MAP_VALUE &_value) {key = _key; value = _value; }
...
struct RBTreeNode : public RBData<MAP_VALUE, MAP_KEY> {
	...
	RBTreeNode *parent;
	bool color;
	RBTreeNode () { color = 0; parent = left = right = 0; };
	~RBTreeNode () {};
	RBTreeNode (RBTreeNode &r) { operator = (r); };
	RBTreeNode *Reset() { RBData<MAP_VALUE, MAP_KEY>::key = 0; color = 0; parent = 0; left = 0; right = 0; return this; }
	RBTreeNode &operator = (RBTreeNode &r) { RBData<MAP_VALUE, MAP_KEY>::key = r.key; RBData<MAP_VALUE, MAP_KEY>::value = r.value; color = r.color; parent = r.parent; left = r.left; right = r.right; return *this; };
	...
	void Init(RBTreeNode *_left, RBTreeNode *_right, RBTreeNode *_parent, bool _color, MAP_KEY &_key, MAP_VALUE &_value)
	{
		left = _left;
		right = _right;
		parent = _parent;
...
struct MMapCmp {
	int compare(LPCSTR a, LPCSTR b) { return strcmp(a, b);	}
	int compare(LPCWSTR a, LPCWSTR b) { return wcscmp(a, b);	}
};
...
public:
	...
	RBMap(LIST_ALLOC *allocator = 0) { if (!allocator) m_Alloc = this; else m_Alloc = allocator;  m_fKeyFromCompareFunc = 0; m_fNewDelele = 1; pRoot = &m_Nil; m_Nil.Init(&m_Nil, &m_Nil, &m_Nil, _BLACK); };
	virtual ~RBMap() { Clear(); };
	RBMap(RBMap &r) { m_Alloc = this; m_fKeyFromCompareFunc = 0; pRoot = &m_Nil; m_Nil.Init(&m_Nil, &m_Nil, &m_Nil, _BLACK); *this = r; } 
	void SetAllocator(LIST_ALLOC *allocator) { Clear(); if (!allocator) m_Alloc = this; else m_Alloc = allocator;   };
	LIST_ALLOC *GetAllocator() { return m_Alloc; }
	...
	virtual ListNotNewEntry<RBTreeNode<MAP_VALUE, MAP_KEY> > *First() { return m_Alloc == this ? LIST_ALLOC::First() : m_Alloc->First(); }
	virtual ListNotNewEntry<RBTreeNode<MAP_VALUE, MAP_KEY> > *Last() { return m_Alloc == this ? LIST_ALLOC::Last() : m_Alloc->Last(); }
	RBTreeNode<MAP_VALUE, MAP_KEY> *Root() { return pRoot; }
	RBTreeNode<MAP_VALUE, MAP_KEY> *Nil() { return &m_Nil; }
	MAP_VALUE &Add(MAP_KEY key, MAP_VALUE &b) { return Add(key, &b); }
	bool Lookup(MAP_KEY key, MAP_VALUE &b);
	bool Lookup(MAP_KEY key, MAP_VALUE **b = 0);
	void Delete(MAP_KEY key) { RBTreeNode<MAP_VALUE, MAP_KEY> *p = Search(pRoot, key); if (p != &m_Nil) DeleteNode(p); }
	RBData<MAP_VALUE, MAP_KEY> *Get(MAP_KEY key) { return (RBData<MAP_VALUE, MAP_KEY> *)Search(pRoot, key); }
	DINT GetLen() { return m_Alloc->Len(); }
	RBMap &operator = (RBMap &a);
	void Clear();
	void Reset();
	MAP_KEY MinKey() { 	RBTreeNode<MAP_VALUE, MAP_KEY> *node = pRoot; while (node->left != &m_Nil)	node = node->left; return node->key; }
	MAP_KEY MaxKey() { 	RBTreeNode<MAP_VALUE, MAP_KEY> *node = pRoot; while (node->right != &m_Nil) node = node->right; return node->key; }
	const RBData< MAP_VALUE, MAP_KEY> *Min() { RBTreeNode<MAP_VALUE, MAP_KEY> *node = pRoot; while (node->left != &m_Nil)	node = node->left; return node; }
	const RBData< MAP_VALUE, MAP_KEY> *Max() { RBTreeNode<MAP_VALUE, MAP_KEY> *node = pRoot; while (node->right != &m_Nil) node = node->right; return node; }
	...
	int Rank() 	{ int y = 0; Rank(y, pRoot);  return y; }
protected:
	...
	virtual RBTreeNode<MAP_VALUE, MAP_KEY> *Search(RBTreeNode<MAP_VALUE, MAP_KEY> *node, MAP_KEY &key);
...
template <class MAP_VALUE, class MAP_KEY>
class RBMap2 : public RBMap<MAP_VALUE, MAP_KEY, RBCmp<MAP_KEY>, ListNotNew2<RBTreeNode<MAP_VALUE, MAP_KEY> > > {
public:
	RBMap2():RBMap<MAP_VALUE, MAP_KEY, RBCmp<MAP_KEY>, ListNotNew2<RBTreeNode<MAP_VALUE, MAP_KEY> > >(0) {};
	RBMap2(RBMap2 &a) : RBMap<MAP_VALUE, MAP_KEY, RBCmp<MAP_KEY>, ListNotNew2<RBTreeNode<MAP_VALUE, MAP_KEY> > >(a) {};
	MAP_VALUE &Value (MAP_KEY key) { return AddSort(key, 0); }
	MAP_VALUE &Add(MAP_KEY key, MAP_VALUE &b) { return AddSort(key, &b); }
	MAP_VALUE &operator [] (MAP_KEY key) { return AddSort(key, 0); }
	RBTreeNode<MAP_VALUE, MAP_KEY> *FirstNode() { return ListNotNew2<RBTreeNode<MAP_VALUE, MAP_KEY> >::m_pFirst; }
	RBTreeNode<MAP_VALUE, MAP_KEY> *LastNode() { return ListNotNew2<RBTreeNode<MAP_VALUE, MAP_KEY> >::m_pLast; }
...
template <class MAP_VALUE, class MAP_KEY, class CMP, class LIST_ALLOC>
void RBMap<MAP_VALUE,  MAP_KEY, CMP, LIST_ALLOC>::GetSortedPtrRBData(MArray<RBData<MAP_VALUE, MAP_KEY> *> &arr)
{
...
//-------------------------------------
template <class MAP_VALUE, class MAP_KEY, class CMP, class LIST_ALLOC>
MAP_VALUE &RBMap<MAP_VALUE,  MAP_KEY, CMP, LIST_ALLOC>::Add(MAP_KEY &key, MAP_VALUE *value, RBTreeNode<MAP_VALUE, MAP_KEY> **_node)
{
...
//----------------------------------------------
template <class MAP_VALUE, class MAP_KEY, class CMP, class LIST_ALLOC>
inline RBTreeNode<MAP_VALUE, MAP_KEY> *RBMap<MAP_VALUE,  MAP_KEY, CMP, LIST_ALLOC>::Search(RBTreeNode<MAP_VALUE, MAP_KEY> *node, MAP_KEY &key)
{
...
//----------------------------------------------
template <class MAP_VALUE, class MAP_KEY, class CMP, class LIST_ALLOC>
inline void RBMap<MAP_VALUE,  MAP_KEY, CMP, LIST_ALLOC>::FreeNode(RBTreeNode<MAP_VALUE, MAP_KEY> *node)
{
...
//----------------------------------------------
template <class MAP_VALUE, class MAP_KEY, class CMP, class LIST_ALLOC>
inline bool RBMap<MAP_VALUE,  MAP_KEY, CMP, LIST_ALLOC>::AllocNode(RBTreeNode<MAP_VALUE, MAP_KEY> **node)
{
	*node = m_Alloc->Add();
	if (*node)
	{
...
//----------------------------------------------
template <class MAP_VALUE, class MAP_KEY, class CMP, class LIST_ALLOC>
bool RBMap<MAP_VALUE,  MAP_KEY, CMP, LIST_ALLOC>::Insert(MAP_KEY &key, MAP_VALUE *value, RBTreeNode<MAP_VALUE, MAP_KEY> **node)
{
...
//----------------------------------------------
template <class MAP_VALUE, class MAP_KEY, class CMP, class LIST_ALLOC>
inline void RBMap<MAP_VALUE,  MAP_KEY, CMP, LIST_ALLOC>::LRotate (RBTreeNode<MAP_VALUE, MAP_KEY> *node)
{
...
//---------------------------------------
template <class MAP_VALUE, class MAP_KEY, class CMP, class LIST_ALLOC>
RBMap<MAP_VALUE,MAP_KEY, CMP, LIST_ALLOC> &RBMap<MAP_VALUE,MAP_KEY, CMP, LIST_ALLOC>::operator = (RBMap<MAP_VALUE,MAP_KEY,CMP, LIST_ALLOC> &a) 
{ 
	Clear();
...
MAP_VALUE &RBMap2<MAP_VALUE, MAP_KEY>::AddSort(MAP_KEY &key, MAP_VALUE *b)
{
	...
	RBMap<MAP_VALUE, MAP_KEY, RBCmp<MAP_KEY>, ListNotNew2<RBTreeNode<MAP_VALUE, MAP_KEY> > >::Add(key, b, &node);
...
template <class MAP_VALUE, class MAP_KEY, class CMP, class LIST_ALLOC>
inline void RBMap<MAP_VALUE,  MAP_KEY, CMP, LIST_ALLOC>::GetSortedRBData(MVector<RBData<MAP_VALUE, MAP_KEY> > &arr) { ::GetSortedRBData(arr, *this); }
template <class MAP_VALUE, class MAP_KEY, class CMP, class LIST_ALLOC>
inline void RBMap<MAP_VALUE,  MAP_KEY, CMP, LIST_ALLOC>::GetSortedRBData(MArray<RBData<MAP_VALUE, MAP_KEY> > &arr) { ::GetSortedRBData(arr, *this); }
template <class MAP_VALUE, class MAP_KEY, class CMP, class LIST_ALLOC>
inline void RBMap<MAP_VALUE,  MAP_KEY, CMP, LIST_ALLOC>::GetSortedKeys(MArray<MAP_KEY> &arr) { ::GetSortedKeys(arr, *this); }
