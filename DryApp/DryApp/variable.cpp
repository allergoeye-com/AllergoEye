...
void AddHex(DWORD d, int sz, TString &hex)
{
	...
	if (sz == 2) 
		sprintf_s(str, 16, sz_2, (BYTE)d);
	else
		sprintf_s(str, 16, sz_8, d);
	hex += str;
	hex.Upper();
}
...
void Variable::UnSerialize(BYTE *b, int blen, int &pos, UINT &algnBuffer, BOOL fNoUnserializeBuffer)
{
	...
	if (m_Type != VAR_EMPTY && m_Type != (TYPE_VAR)Mem2Long(ptr + pos))
		Clear();
	m_Type = (TYPE_VAR)Mem2Long(ptr + pos); pos += sizeof (TYPE_VAR);
  ...
  //
	...
	case VAR_BUFFER:
	{
		 if (pos + sizeof (int) > (UINT)blen)
			goto error;
		int len = Mem2Int(ptr + pos); pos += sizeof (int);
		 if (pos + len > blen)
			goto error;
		if (!m_Data.buff)
		{
	...
	case VAR_GUID :
		...
		if (!m_Data.guid)
			m_Data.guid = new GUID;
		memcpy(m_Data.guid, ptr + pos, sizeof (GUID)); pos += sizeof (GUID);
		break;

	case VAR_CHAR :
		if (pos + sizeof (char) > (UINT)blen)
			goto error;
		m_Data.ch = *(char *)(ptr + pos); ++pos;
		break;
	case VAR_UCHAR :
		if (pos + sizeof (char) > (UINT)blen)
			goto error;
		m_Data.ch = *(ptr + pos); ++pos;
		break;
	case VAR_DOUBLE:
	...
	case VAR_SHORT:
		if (pos + sizeof (short) > (UINT)blen)
			goto error;
		m_Data.sh = Mem2Short(ptr + pos);  pos += sizeof(m_Data.sh);
		break;
	case VAR_LONG:
	case VAR_ULONG:
	case VAR_INT:
	...
	case VAR_ALIGNMENT:
		if (pos + sizeof (int) > (UINT)blen)
			goto error;
		m_Data.lg  = Mem2Long(ptr + pos);  pos += sizeof(int);
		break;
	default:
	error:
		Clear();	
...
TString Variable::GetHexString()
{
	...
	{
		DWORD addr = 0;
		MakeHex((BYTE *)m_Data.wstr->GetPtr(), m_Data.wstr->GetLen() * sizeof(wchar_t), ret, &addr);
	}
	break;
	}
	return ret;
...
//
//------------------------------------------
void Variable::UnSerialize(MFILE *hFile, int blen, int &pos, UINT &algnBuffer, BOOL fNoUnserializeBuffer)
{
 	//Clear();
...
void Variable::SizeOf(int &size)
{
	...
	case VAR_WSTRING:
	{
		wchar_t *wstr = m_Data.wstr->GetPtr();
		if (!wstr || !wcslen(wstr))
			size += (int)sizeof(wchar_t);
		else
			size += (int)(sizeof(wchar_t) * (wcslen(wstr) + 1));
	}
	break;
	case VAR_GUID:
		size += sizeof(GUID);
