...
public:
	...
	void CreateNew(BOOL b) {m_fNewDelele = b; }
	...
	DINT FindIndex (MAP_KEY key) { DINT p = Search(pRoot, key); return p; }
	...
	void PreAlloc(DINT len) { Data.SetLen(len); Data.SetLen(1); Tree.SetLen(len); Tree.SetLen(1);
								bData = Data.GetPtr() + 1;
								bTree = Tree.GetPtr() + 1;
							}
	...
	MAP_KEY *VData(int &len) { len = Data.GetLen() - 1; return bData; }
	ArRBTreeNode *VTree(int &len) { len = Tree.GetLen() - 1; return bTree; }
