...
public:
	ListAllocator() {};
	virtual ~ListAllocator() { Clear(); }
	virtual void Delete(T*) {}
	virtual void Clear() {};
	virtual void Reset() { Clear(); }
	virtual T *Add() { return 0; }
	virtual T *Add(bool) { return 0; }
	virtual void RunAllDataDestructor() {};
	virtual MListEntry<T> *Last() { return 0; }
	virtual MListEntry<T> *First() { return 0; }
	virtual DINT Len() { return 0; }
...
struct MList : public ListAllocator<T> {
	...
	MList(MList &a) : m_pFirst(0),m_pLast(0),m_iNum(0),m_fKill(true) { operator = (a); };
	virtual ~MList() { Clear(); }
	...
	virtual void Clear();
	virtual T *Add() { return Add((T *)0); }
	T *Add(T *a);
 	T *Insert(DINT i, T *a=0);
	virtual DINT Len() { return m_iNum; }
	T *Add(MListEntry<T> *entry);
	virtual MListEntry<T> *First() { return m_pFirst; }
	virtual MListEntry<T> *Last() { return m_pLast; }
...
public:
	...
	virtual ~TemplateListNotNew() { Clear(); }
	virtual void Delete(T *b);
	virtual void Clear();
	virtual DINT Len() { return m_iNum; }
	virtual T *Add();
	virtual ListNotNewEntry<T> *Last() { return m_pLast; }
	virtual ListNotNewEntry<T> *First() { return m_pFirst; }
	virtual void RunAllDataDestructor() {
...
public:
	ListNotNew() {  };
	virtual ~ListNotNew() {  }
	virtual void Clear() { TemplateListNotNew<T>::Clear(); 	m_TAlloc.Clear(); m_mAlloc.Clear(); }
	virtual void Reset()
	{
		m_mAlloc.FreeAll();
		m_TAlloc.FreeAll();
...
protected:
	virtual ListNotNewEntry<T> *AllocListEntry() { return m_mAlloc.Alloc(0); }
	virtual T *AllocData() { return m_TAlloc.Alloc(0); }
	virtual void Free(T *t)  { m_TAlloc.Free(t); }
	virtual void Free(ListNotNewEntry<T> *t) { m_mAlloc.Free(t); }
protected:
...
public:
	AllocatorSimple() {m_iNum = 0;};
	virtual ~AllocatorSimple() { Clear(); }
	virtual void Delete(T *t) { if (m_iNum) {--m_iNum;  m_mAlloc.Free(t); } }
	virtual void Clear() {m_mAlloc.Clear(); m_iNum = 0; };
	virtual void Reset() { m_mAlloc.FreeAll(); m_iNum = 0; }
	virtual T *Add() { ++m_iNum; return m_mAlloc.Alloc(0); }
	virtual void RunAllDataDestructor();
	virtual MListEntry<T> *Last() { return 0; }
	virtual MListEntry<T> *First() { return 0; }
	virtual DINT Len() { return m_iNum; }
...
public:
	...
	ListNotNewEntry<T> *Last() { return 0; }
	ListNotNewEntry<T> *First() { return 0; }
	virtual DINT Len() { return m_iNum; }
...
void AllocatorSimple<T>::RunAllDataDestructor() 
{
	...
	if (ln)
	{
		...
		while(ln--)
		{
			...
			if ((DINT)m_mAlloc._Max_Alloc_ != -1 && n > m_mAlloc._Max_Alloc_)
...
MList<T> &MList<T>::operator = (const MList<T> &p)
{
	...
	m_fKill = true;
	for (DINT i = 0; i < p.m_iNum; ++i, entry = entry->m_pNext, m_entry = i < m_iNum ? m_entry->m_pNext : 0 )
	{
		if (m_entry) 
