...
struct DataLoopEntry {
	LoopEntry *self;
	DataLoopEntry() { self = 0; }
};
struct LoopEntry {
	LoopEntry *left;
	LoopEntry *right;
	DataLoopEntry *data; 
	LoopEntry() { data = 0; left = right = this; };
	void Init(LoopEntry *_left, LoopEntry *_right, DataLoopEntry *_data = 0)
	{
		left = _left; right = _right; data = _data; if (data) data->self = this; 
	};
...
public:
	LoopList () { pMin = 0;	Num = 0; m_DefaultAlloc = 4096; }
	~LoopList()
	{ 
	...
	DataLoopEntry *Min() { return pMin ?  pMin->left->data : 0; }
	DataLoopEntry *Max() { return pMin ?  pMin->right->data : 0; }
	bool IsEmpty() { return Num == 0; }
	DataLoopEntry *RemoveMin() { if (!pMin) return 0;DataLoopEntry *x = pMin->left->data; Remove(pMin->left->data); return x; }
	DINT Len() const { return Num; } 
...
private:
	DWORD private_allocNodes(BYTE **p)
	{
		...
		DWORD n = (DWORD)1 << ((lckn > 23 ? 23 : lckn)  + 1);
