...
struct LineItem : public Segment2D {
	...
	LineItem() {iIndex = -1; }
	~LineItem() {}
	LineItem(const Vector2D &sorg, const Vector2D &sdest, int i) { org = sorg; dest = sdest; iIndex = i; }

	void Set(Segment2D &s, int i) { org = s.org; dest = s.dest; iIndex = i; }
};
...
struct ListLine : public MDeq<LineItem> {
	...
	ListLine(ListLine &f) 
	{
		fKill = true; m_iNum = 0; m_Al = 0; last = first = 0; *this = f;
	}
...
{
public:
	RuningPoygon() { iIndex = -1; };
	~RuningPoygon() {};
	RuningPoygon(const RuningPoygon &p) { list = p.list;  box = p.box; iIndex = p.iIndex; }

	void Clip(Box2D &_box);

	void Reset() { box.Reset(); list.Clear();   }
	RuningPoygon &operator = (const RuningPoygon &p) { Reset();  list = p.list; box = p.box;  iIndex = p.iIndex; return *this; }
...
bool IsRight(RuningPoygon *pe);
	...
	bool IsClipXRay(Segment2D &p, Vector2D pt[2]);
