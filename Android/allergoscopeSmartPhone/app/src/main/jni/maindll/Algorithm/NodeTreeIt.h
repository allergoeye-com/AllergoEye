...
//	DINT pw;
    ...
    void Set(DINT v, DINT w, double _pwt)
    {
        pv = v;
        pw = w;
        pwt = _pwt;
...
public:
    HeapEdge() : ptr(MArray<GNodeW>::ptr)
    {
        iRank = 1;
        ffFix = false;
        N = 0;
...
{
    HMapTreeNode<DINT, GraphEdge> *node = MMalloc<HMapTreeNode<DINT, GraphEdge> *>::ptr[GetHash(
            key)];
...
public:
    HeapVertex() : ptr(MArray<GNodeW>::ptr)
    {
        iRank = 1;
        ffFix = false;
        N = 0;
...
public:
    HeapVertexInv() : ptr(MArray<GNodeW>::ptr)
    {
        iRank = 1;
        ffFix = false;
        N = 0;
...
public:
    NodeTreeIt_iterator(NodeTreeIt *_tree, DINT len, DINT _pv)
    {
        N = len;
        tree = _tree;
        pv = _pv;
