...
//
//-------------------------------------------------------------------------------------
void BuilderVoron2D::Build(MArray<Vector2F> &pos, MArray<Vector2F> &minus, int X, int Y,
                           MArray<GNode> &adj)
{
...
//
//-------------------------------------------------------------------------------------
void BuilderVoron2D::BuildGabriel(MArray<Vector2F> &pos, MArray<Vector2F> &minus, int X, int Y,
                                  MArray<GNode> &adj)
{
    Box2D orig(0, 0, X - 1, Y - 1);
    Box2D mk = orig;
    ...
    for (int i = 0; i < len; ++i)
    {
        ...
        while (node)
        {
            if (node->value.iIndex >= 0 && node->value.iIndex < len)
            {
                ...
                while (n1)
                {
                    if (n1->value.iIndex != node->value.iIndex && n1->value.iIndex != i &&
                        n1->value.iIndex >= 0)
                    {
...
//
//-------------------------------------------------------------------------------------
void BuilderVoron2D::Build(MArray<Vector2F> &vpt, MArray<Vector2F> &minus, Box2D &rc,
                           MVector<RuningPoygon> &e, MVector<AllocatorLine> &pAlloc)
{
...
//
//--------------------------------------
void BuilderVoron2D::Build(MArray<Vector2D> &m_Data, Box2D &rc, MVector<RuningPoygon> &e,
                           AllocatorLine &pAlloc)
{
...
//
//--------------------------------------
void BuilderVoron2D::Build2(MArray<Vector2D> &m_Data, Box2D &rc, MVector<RuningPoygon> &e,
                            MVector<AllocatorLine> &pAlloc)
{
...
*/
    ...
    ParamBuldVoron2D *Param = (ParamBuldVoron2D *) _alloca(
            sizeof(ParamBuldVoron2D) * (NThreads + 1));
    Param[0].self = this;
    Param[0].pe = pe;
    Param[0].N = step;
    Param[0].plist = plist;
    ...
    ParamBuldVoron2D_2 *Param2 = (ParamBuldVoron2D_2 *) _alloca(
            sizeof(ParamBuldVoron2D_2) * (NThreads + 1));
...
void
BuilderVoron2D::Build(int i, int len, TListArray<DINT>::node *plist, DINT *pa, RuningPoygon *pe)
{
    TListArray<DINT> run;
    TListArray<DINT> list_run;
    int j = 0;
