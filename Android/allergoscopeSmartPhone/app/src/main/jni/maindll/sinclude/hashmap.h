...
struct GraphEdge {
    ...
    void Set(DINT _pv, DINT _pw)
    {
        pv = _pv;
        pw = _pw;
    }

    GraphEdge &Init(DINT _pv, DINT _pw)
    {
        pv = _pv;
        pw = _pw;
        return *this;
    ...
    { return pv > e.pv || (pv == e.pv && pw > e.pw); }

    bool operator<(const GraphEdge &e)
    { return pv < e.pv || (pv == e.pv && pw < e.pw); }

    bool operator==(const GraphEdge &e)
    { return pv == e.pv && pw == e.pw; }
...
struct HMapTreeNode : public RBData<MAP_VALUE, MAP_KEY> {
    ...
    HMapTreeNode(HMapTreeNode &r)
    { operator=(r); };

    HMapTreeNode &operator=(HMapTreeNode &r)
    {
...
public:
    MHashMap(int _iRank = 2, int max_alloc_block = 1024, int min_alloc_block = 4,
             int _iStartSize = 128)
    {
    ...
    HMapTreeNode<MAP_VALUE, MAP_KEY> **GetRoot()
    { return ARRAY::GetPtr(); }
    ...
    DINT Len() const
    { return LIST_ALLOC::m_iNum; }

    virtual MListEntry<HMapTreeNode<MAP_VALUE, MAP_KEY> > *Last()
    { return LIST_ALLOC::Last(); }

    virtual MListEntry<HMapTreeNode<MAP_VALUE, MAP_KEY> > *First()
    { return LIST_ALLOC::First(); }
...
protected:
    ...
    bool SetRoot(HMapTreeNode<MAP_VALUE, MAP_KEY> **root, MAP_KEY &key, MAP_VALUE *value,
                 HMapTreeNode<MAP_VALUE, MAP_KEY> **node);

    bool AllocNode(MAP_KEY &key, MAP_VALUE *value, HMapTreeNode<MAP_VALUE, MAP_KEY> **node);

    bool Insert(HMapTreeNode<MAP_VALUE, MAP_KEY> **root, MAP_KEY &key, MAP_VALUE *value,
                HMapTreeNode<MAP_VALUE, MAP_KEY> **node);

    void GetData(HMapTreeNode<MAP_VALUE, MAP_KEY> *root,
                 MArray<HMapTreeNode<MAP_VALUE, MAP_KEY> *> &stack,
                 MArray<HMapTreeNode<MAP_VALUE, MAP_KEY> *> &res);
    ...
    int GetHash(GraphEdge &p)
    { return GetHash(p.pv + p.pw); }
    ...
    int GetHash(size_t p) const
    { return _GetHash((DWORD) p); }
...
class RBHashMap
        : protected MMalloc<RBTreeNode<MAP_VALUE, MAP_KEY> *>,
          protected RBMap<MAP_VALUE, MAP_KEY, CMP, LIST_ALLOC> {
public:
    ...
    RBHashMap(int start_hash_size = 128, int _iRank = 2, int max_alloc_block = 1024,
              int min_alloc_block = 4)
    {
    ...
    void CreateNew(bool fNewDelele)
    { RBMMAPH::m_fNewDelele = fNewDelele; }

    void KeyFromCompareFunc(bool b)
    { RBMMAPH::m_fKeyFromCompareFunc = b; }
    ...
    RBTreeNode<MAP_VALUE, MAP_KEY> **GetRoot()
    { return MMalloc<RBTreeNode<MAP_VALUE, MAP_KEY> *>::GetPtr(); }

    int GetLenRoot()
    { return MMalloc<RBTreeNode<MAP_VALUE, MAP_KEY> *>::GetLen(); }
...
protected:
    ...
    void
    GetData(RBTreeNode<MAP_VALUE, MAP_KEY> *root, MArray<RBTreeNode<MAP_VALUE, MAP_KEY> *> stack,
            MArray<RBTreeNode<MAP_VALUE, MAP_KEY> *> &res);
...
public:
    ...
    class iterator {
    public:
        iterator(RBHashMap &m) : M(m)
        {
            hi = M.RBMALLOCH::GetLen();
            it = new RBiterator(m);
...
template<class MAP_VALUE, class MAP_KEY, class ARRAY, class LIST_ALLOC>
inline void
MHashMap<MAP_VALUE, MAP_KEY, ARRAY, LIST_ALLOC>::FreeNode(HMapTreeNode<MAP_VALUE, MAP_KEY> *node)
{
...
template<class MAP_VALUE, class MAP_KEY, class ARRAY, class LIST_ALLOC>
inline bool
MHashMap<MAP_VALUE, MAP_KEY, ARRAY, LIST_ALLOC>::AllocNode(HMapTreeNode<MAP_VALUE, MAP_KEY> **node)
{
    *node = LIST_ALLOC::Add();
    if (*node)
    {
...
template<class MAP_VALUE, class MAP_KEY, class ARRAY, class LIST_ALLOC>
inline bool
MHashMap<MAP_VALUE, MAP_KEY, ARRAY, LIST_ALLOC>::SetRoot(HMapTreeNode<MAP_VALUE, MAP_KEY> **root,
                                                         MAP_KEY &key, MAP_VALUE *value,
                                                         HMapTreeNode<MAP_VALUE, MAP_KEY> **node)
{
    if (*node)
        *root = *node;
...
template<class MAP_VALUE, class MAP_KEY, class ARRAY, class LIST_ALLOC>
inline bool
MHashMap<MAP_VALUE, MAP_KEY, ARRAY, LIST_ALLOC>::AllocNode(MAP_KEY &key, MAP_VALUE *value,
                                                           HMapTreeNode<MAP_VALUE, MAP_KEY> **node)
{
    if (!*node)
    {
...
template<class MAP_VALUE, class MAP_KEY, class ARRAY, class LIST_ALLOC>
void
MHashMap<MAP_VALUE, MAP_KEY, ARRAY, LIST_ALLOC>::Delete(HMapTreeNode<MAP_VALUE, MAP_KEY> **root,
                                                        MAP_KEY &key)
{
...
template<class MAP_VALUE, class MAP_KEY, class ARRAY, class LIST_ALLOC>
bool
MHashMap<MAP_VALUE, MAP_KEY, ARRAY, LIST_ALLOC>::Insert(HMapTreeNode<MAP_VALUE, MAP_KEY> **root,
                                                        MAP_KEY &key, MAP_VALUE *value,
                                                        HMapTreeNode<MAP_VALUE, MAP_KEY> **node)
{
...
template<class MAP_VALUE, class MAP_KEY, class ARRAY, class LIST_ALLOC>
inline void
MHashMap<MAP_VALUE, MAP_KEY, ARRAY, LIST_ALLOC>::Build(MArray<RBData<MAP_VALUE, MAP_KEY> > &ar)
{
    Clear();
...
template<class MAP_VALUE, class MAP_KEY, class ARRAY, class LIST_ALLOC>
inline void
MHashMap<MAP_VALUE, MAP_KEY, ARRAY, LIST_ALLOC>::GetData(MArray<RBData<MAP_VALUE, MAP_KEY> > &res)
{
...
template<class MAP_VALUE, class MAP_KEY, class ARRAY, class LIST_ALLOC>
void
MHashMap<MAP_VALUE, MAP_KEY, ARRAY, LIST_ALLOC>::GetData(HMapTreeNode<MAP_VALUE, MAP_KEY> *root,
                                                         MArray<HMapTreeNode<MAP_VALUE, MAP_KEY> *> &stack,
                                                         MArray<HMapTreeNode<MAP_VALUE, MAP_KEY> *> &res)
{
    bool fCheck = true;
...
//-------------------------------------------------------
template<class MAP_VALUE, class MAP_KEY, class ARRAY, class LIST_ALLOC>
inline MHashMap<MAP_VALUE, MAP_KEY, ARRAY, LIST_ALLOC> &
MHashMap<MAP_VALUE, MAP_KEY, ARRAY, LIST_ALLOC>::operator=(
        MHashMap<MAP_VALUE, MAP_KEY, ARRAY, LIST_ALLOC> &m)
{
    Clear();
    iMaxSize = m.iMaxSize;
...
template<class MAP_VALUE, class MAP_KEY, class ARRAY, class LIST_ALLOC>
inline bool MHashMap<MAP_VALUE, MAP_KEY, ARRAY, LIST_ALLOC>::operator==(
        MHashMap<MAP_VALUE, MAP_KEY, ARRAY, LIST_ALLOC> &m)
{
...
template<class MAP_VALUE, class MAP_KEY, class ARRAY, class LIST_ALLOC>
inline MAP_VALUE &
MHashMap<MAP_VALUE, MAP_KEY, ARRAY, LIST_ALLOC>::Add(MAP_KEY &key, MAP_VALUE &value)
{
...
inline void
RBHashMap<MAP_VALUE, MAP_KEY, CMP, LIST_ALLOC>::Build(MArray<RBData<MAP_VALUE, MAP_KEY> > &ar)
{
    Clear();
...
template<class MAP_VALUE, class MAP_KEY, class CMP, class LIST_ALLOC>
inline void
RBHashMap<MAP_VALUE, MAP_KEY, CMP, LIST_ALLOC>::GetData(MArray<RBData<MAP_VALUE, MAP_KEY> > &res)
{
    res.SetLen(LIST_ALLOC::m_iNum);
    RBData<MAP_VALUE, MAP_KEY> *ptab = res.GetPtr();
    iterator it(*this);
...
template<class MAP_VALUE, class MAP_KEY, class CMP, class LIST_ALLOC>
void RBHashMap<MAP_VALUE, MAP_KEY, CMP, LIST_ALLOC>::GetData(RBTreeNode<MAP_VALUE, MAP_KEY> *root,
...
//-------------------------------------------------------
template<class MAP_VALUE, class MAP_KEY, class CMP, class LIST_ALLOC>
inline RBHashMap<MAP_VALUE, MAP_KEY, CMP, LIST_ALLOC> &
RBHashMap<MAP_VALUE, MAP_KEY, CMP, LIST_ALLOC>::operator=(
        RBHashMap<MAP_VALUE, MAP_KEY, CMP, LIST_ALLOC> &m)
{
    Clear();
    iMaxSize = m.iMaxSize;
    ReAlloc(m.Len);
...
template<class MAP_VALUE, class MAP_KEY, class CMP, class LIST_ALLOC>
inline MAP_VALUE &
RBHashMap<MAP_VALUE, MAP_KEY, CMP, LIST_ALLOC>::Add(MAP_KEY &key, MAP_VALUE &value)
{
    MAP_VALUE &val = Value(key);
    val = value;
    return val;
...
inline int RBHashMap<MAP_VALUE, MAP_KEY, CMP, LIST_ALLOC>::_GetHash(DWORD ID) const
{
    ...
    return (int)(ID & iMask);
#else
    return (DWORD) ((ID % (DWORD) MMalloc<RBTreeNode<MAP_VALUE, MAP_KEY> *>::Len +
                     ID / (DWORD) MMalloc<RBTreeNode<MAP_VALUE, MAP_KEY> *>::Len) %
                    (DWORD) MMalloc<RBTreeNode<MAP_VALUE, MAP_KEY> *>::Len);
#endif
}
