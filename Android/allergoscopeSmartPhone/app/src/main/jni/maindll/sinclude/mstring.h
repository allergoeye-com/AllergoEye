...
DINT MultiByteToWideChar(int ignore, int ignore1, const char *src, DINT srclength, WCHAR *dst,
                         DINT dstlength);

DINT WideCharToMultiByte(int ignore, int ignore1, const WCHAR *src, DINT srclength, char *dst,
                         DINT dstlength, const char *ignore2, int *ignore3);
...
//
//---------------------------------------------
inline const wchar_t *__T2U(const char *src, wchar_t *dst, int n)
{
    if (dst != 0L && src != 0L)
    {
...
#ifdef WIN32
//------------------------------------------------------------
inline int __T2ULen(const char *src)
{
    int len = 0;
    if (src != 0L)
    {
...
//
//---------------------------------------------
inline const char *__U2T(const wchar_t *src, char *dst, int n)
{
    if (dst != 0L && src != 0L)
    {

#ifdef _WIN32_WCE
        DINT len = (DINT)wcslen(src);
...
//
//---------------------------------------------
inline int __U2TLen(const wchar_t *src)
{
    int len = 0;
    if (src != 0L)
    {
...
public:
    ...
    TmString(const string &s)
    { *this = s.c_str(); };

    TmString &operator=(const string &s)
    {
        *this = s.c_str();
        return *this;
    };
    ...
    TmString(const T &s) : T(s)
    {
        T::SetLen(T::Len + 1);
        T::ptr[T::Len - 1] = 0;
    };

    TmString(const TmString &s) : T(s)
    {};

    TmString(const char *s);

    TmString(const wchar_t *s);

    ~TmString()
    {};

    TmString(char ch, int nRepeat = 1)
    {
        T::SetLen(nRepeat + 1);
        memset(T::ptr, ch, nRepeat);
    }

    TmString(const char *lpch, DINT nLength)
    {
        T::SetLen(nLength + 1);
        memcpy(T::ptr, lpch, nLength);
    }

    int GetLength() const
    { return T::Len && T::ptr ? (int) strlen(T::ptr) : 0; }

    char &operator[](int i) const
    { return T::ptr[i]; }

    BOOL IsEmpty() const
    { return !T::ptr || !T::ptr[0]; }
    ...
    int ReadInt(DINT offset)
    { return offset + sizeof(int) < T::Len ? Mem2Int((BYTE *) T::ptr + offset) : 0; }

    WORD ReadWord(DINT offset)
    { return offset + sizeof(WORD) < T::Len ? Mem2Short((BYTE *) T::ptr + offset) : 0; }

    BYTE ReadByte(DINT offset)
    { return offset + sizeof(BYTE) < T::Len ? *((BYTE *) T::ptr + offset) : 0; }
    ...
    int ToInt()
    { return StrLen() ? atoi(T::ptr) : 0; }

    double ToDouble()
    { return StrLen() ? atof(T::ptr) : 0; }

    bool ToDouble(double *db)
    {
        int fRet = 0;
        if (StrLen()) fRet = sscanf(T::ptr, "%lf", db);
        return fRet == StrLen();
    }
    ...
    {
        int fRet = 0;
        if (StrLen()) fRet = sscanf(T::ptr, "%d", db);
        return fRet == StrLen();
    }

    char GetAt(DINT nIndex)
    { return T::ptr[nIndex]; }

    void SetAt(DINT nIndex, char ch)
    { T::ptr[nIndex] = ch; }

    const TmString &operator=(char ch)
    {
        T::SetLen(2);
        T::ptr[0] = ch;
        T::ptr[1] = 0;
    ...
    DINT Delete(DINT nIndex, DINT nCount = 1)
    {
        T::Remove(nIndex, nCount);
        if (T::ptr)T::ptr[T::Len - 1] = 0;
        return T::Len;
    }

    DINT Remove(DINT nIndex, DINT nCount)
    {
        T::Remove(nIndex, nCount);
        if (T::ptr)T::ptr[T::Len - 1] = 0;
        return T::Len;
    ...
    void ReleaseBuffer(DINT nNewLength = 0)
    { T::SetLen(nNewLength); }

    char *GetBufferSetLength(DINT nNewLength);

    TmString &operator=(const T &s)
    {
        T::operator=(s);
        if (!T::Len && T::ptr)T::ptr[0] = 0;
        return *this;
    }
    ...
    TmString &operator+=(USHORT s)
    { return operator+=((UINT) s); }

    TmString &operator+=(int s)
    {
        char str[16];
        sprintf(str, "%d", s);
        operator+=(str);
    ...
    TmString &operator+=(short s)
    { return operator+=((int) s); }

    TmString &operator+=(float s)
    {
        char str[64];
        sprintf(str, "%.4f", s);
        operator+=(str);
    ...
    BOOL operator>(const TmString &s) const
    {
        if (T::Len && !s.Len) return true;
        if (s.Len && !T::Len) return false;
        return strcmp(T::ptr, s.ptr) > 0;
    }
    ...
    BOOL operator>(const char *s) const
    {
        ...
        if (s && strlen(s) && !T::Len) return false;
    ...
    BOOL operator<(const char *s) const
    {
        ...
        if (s && strlen(s) && !T::Len) return true;
        return strcmp(T::ptr, s) < 0;
    }
    ...
    DINT Insert(DINT nIndex, TmString &str)
    { return Insert(nIndex, str.ptr); }
    ...
    void Upper()
    { for (int i = 0, len = StrLen(); i < len; ++i) T::ptr[i] = toupper(T::ptr[i]); }

    void Lower()
    { for (int i = 0, len = StrLen(); i < len; ++i) T::ptr[i] = tolower(T::ptr[i]); }
    ...
    MVector<TmString> Tokenize(const char *ps, bool fAll = true, bool fClear0 = true)
    {
        MVector<TmString> v;
        Tokenize(ps, v, fAll, fClear0);
        return v;
    };
    ...
    TmString Tail(DINT i)
    { return TmString(T::Tail(i)); }

    TmString Left(DINT i)
    { return TmString(T::Left(i)); }

    TmString Right(DINT i)
    { return TmString(T::Right(i + T::Len - StrLen())); }

    TmString Mid(DINT first, DINT count = -1)
    {
        if (!T::ptr) count = 0; else if (count == -1) count = strlen(T::ptr) - first;
        if (!count) return TmString();
        return TmString(T::Mid(first, count));
    }
    ...
    {
        DINT n = StrLen();
        if (n > 1) T::ReOrder(0, n - 1);
    }

    void ClearSpace()
    { Trim(); }

    DINT GetAllocLength() const
    { return T::Len; }
...
#endif
private :
    bool _copy(const char *b);

    bool _copy(const wchar_t *b);

};


template<class T>
TmString<T>::TmString(const char *b) :T()
{
    int n;
    if (b && (n = (int) strlen(b)))
    {
...
template<class T>
TmString<T>::TmString(const wchar_t *s) :T()
{ _copy(s); };
...
public:
    ...
    WTmString(const wchar_t *s) : T()
    { *this = s; };

    WTmString(const char *s) : T()
    { *this = s; };

    ~WTmString()
    {};

    WTmString(wchar_t ch, int nRepeat = 1)
    {
        T::SetLen(nRepeat + 1);
        for (DINT i = 0; i < nRepeat; ++i) T::ptr[i] = ch;
    }

    WTmString(const wchar_t *lpch, DINT nLength)
    { T::Copy(lpch, nLength); }
    ...
    {
        double n = 0.0;
        if (StrLen()) swscanf(T::ptr, L"%lf", &n);
        return n;
    }
    ...
    {
        DINT fRet = 0;
        if (StrLen()) fRet = swscanf(T::ptr, L"%lf", db);
        return fRet == StrLen();
    }
    ...
    {
        DINT fRet = 0;
        if (StrLen()) fRet = swscanf(T::ptr, L"%d", db);
        return fRet == StrLen();
    }

    wchar_t GetAt(DINT nIndex)
    { return T::ptr[nIndex]; }

    void SetAt(DINT nIndex, wchar_t ch)
    { T::ptr[nIndex] = ch; }

    const WTmString &operator=(wchar_t ch)
    {
        T::SetLen(2);
        T::ptr[0] = ch;
        T::ptr[1] = 0;
    ...
    char *GetBuffer(DINT nMinBufLength)
    {
        if (T::Len < nMinBufLength / sizeof(wchar_t))
            T::SetLen(nMinBufLength / sizeof(wchar_t) + 1);
        return ((char *) T::ptr) + nMinBufLength;
    }

    void ReleaseBuffer(DINT nNewLength = -1)
    { if (nNewLength <= 0) T::Clear(); else T::SetLen(nNewLength); }

    char *GetBufferSetLength(DINT nNewLength)
    {
        T::SetLen(nNewLength / sizeof(wchar_t));
        return T::ptr ? T::ptr : L"";
    }
    ...
    WTmString operator+(WTmString &s)
    {
        WTmString a(*this);
        a += s;
        return a;
    ...
    WTmString &operator+=(UINT s)
    {
        wchar_t str[16];
        swprintf(str, 16, L"%u", s);
        operator+=(str);
        return *this;
    }

    WTmString &operator+=(USHORT s)
    { return operator+=((UINT) s); }

    WTmString &operator+=(int s)
    {
        wchar_t str[16];
        swprintf(str, 16, L"%d", s);
        operator+=(str);
        return *this;
    }

    WTmString &operator+=(short s)
    { return operator+=((int) s); }

    WTmString &operator+=(float s)
    {
        wchar_t str[64];
        swprintf(str, 64, L"%.4f", s);
        operator+=(str);
        return *this;
    }

    WTmString &operator+=(double s)
    {
        wchar_t str[128];
        swprintf(str, 128, L"%f", s);
        operator+=(str);
        return *this;
    }
    ...
    WTmString &operator+=(INT64 s)
    {
        wchar_t str[64];
        swprintf(str, 64, L"%lld", s);
        operator+=(str);
        return *this;
    }
    ...
    BOOL operator==(const WTmString &s) const
    { return Cmp(s.ptr); };
    ...
    {
        if (T::Len && !s.Len) return true;
        if ((s.Len && !T::Len) || (!s.Len && !T::Len)) return false;
        return wcscmp(T::ptr, s.ptr) > 0;
    }

    BOOL operator<(const WTmString &s)
    {
        if ((T::Len && !s.Len) || (!T::Len && !s.Len)) return false;
        if (s.Len && !T::Len) return true;
        return wcscmp(T::ptr, s.ptr) < 0;
    }
    ...
    BOOL operator>(const wchar_t *s) const
    {
        if ((T::Len && !s) || (!T::Len && !s)) return true;
        if ((s && wcslen(s)) && !T::Len) return false;
        return wcscmp(T::ptr, s) > 0;
    }

    BOOL operator<(const wchar_t *s) const
    {
        if ((T::Len && !s) || (!T::Len && !s)) return false;
        if ((s && wcslen(s)) && !T::Len) return true;
        return wcscmp(T::ptr, s) < 0;
    }


    int StrLen()
    { return T::Len && T::ptr ? (int) wcslen(T::ptr) : 0; }

    DINT Insert(DINT nIndex, wchar_t ch);

    DINT Insert(DINT nIndex, const wchar_t *pstr);

    DINT Insert(DINT nIndex, const WTmString &str)
    { return Insert(nIndex, str.ptr); }
    ...
    void Upper()
    { for (DINT i = 0, len = StrLen(); i < len; ++i) T::ptr[i] = towupper(T::ptr[i]); }

    void Lower()
    { for (DINT i = 0, len = StrLen(); i < len; ++i) T::ptr[i] = towlower(T::ptr[i]); }
    ...
    MVector<WTmString> Tokenize(const wchar_t *ps, bool fAll = true, bool fClear0 = true)
    {
        MVector<WTmString> v;
        Tokenize(ps, v, fAll, fClear0);
        return v;
    ...
    void
    Tokenize(const wchar_t *seps, MVector<WTmString> &arr, bool fAll = true, bool fClear0 = true);

    void Tokenize(const wchar_t *seps, MArray<int> &arr, bool fAll = true, bool fClear0 = true);

    WTmString Tail(DINT i)
    { return WTmString(T::Tail(i)); }

    WTmString Left(DINT i)
    { return WTmString(T::Left(i)); }

    WTmString Right(DINT i)
    { return WTmString(T::Right(i + T::Len - StrLen())); }

    WTmString Mid(DINT first, DINT count)
    { return WTmString(T::Mid(first, count)); }
...
template<class T>
char *TmString<T>::GetBuffer(DINT nMinBufLength)
{
...
template<class T>
char *TmString<T>::GetBufferSetLength(DINT nNewLength)
{
    T::SetLen(nNewLength);
    if (!T::ptr)
        T::Add() = 0;
...
//---------------------------------------------
template<class T>
inline DINT TmString<T>::Replace(char chOld, char chNew)
{
    DINT k = 0;
...
//---------------------------------------------
template<class T>
inline DINT TmString<T>::Remove(char chRemove)
{
    DINT k = 0;
    for (DINT i = 0; i < T::Len; ++i)
    {
...
//---------------------------------------------
template<class T>
TmString<T> &TmString<T>::Trim(char ch)
{
    DINT k = StrLen();
    while (k && T::ptr[0] == ch)
    {
...
//---------------------------------------------
template<class T>
TmString<T> &TmString<T>::Trim(const char *ch)
{
...
//---------------------------------------------
template<class T>
inline int TmString<T>::FindOneOf(const char *ch)
{
    if (!ch || !ch[0]) return -1;
    DINT l = StrLen();
    for (DINT i = 0; i < l; ++i)
...
//---------------------------------------------
template<class T>
inline TmString<T> &TmString<T>::TrimLeft(char ch)
{
    DINT k = StrLen();
    while (k && T::ptr[0] == ch)
    {
...
//---------------------------------------------
template<class T>
inline TmString<T> &TmString<T>::TrimRight(char ch)
{
    DINT k = StrLen();
    while (k && T::ptr[k - 1] == ch)
    {
...
//---------------------------------------------
template<class T>
inline int TmString<T>::Find(const char *s, DINT offset)
{
    char *c;
...
//---------------------------------------------
template<class T>
inline int TmString<T>::Find(char s, DINT offset)
{
    char *c;
...
//---------------------------------------------
template<class T>
inline int TmString<T>::ReverseFind(char ch)
{
    char *c;
...
//---------------------------------------------
template<class T>
inline TmString<T> &TmString<T>::operator+=(const TmString<T> &s)
{
    if (s.Len > 1)
    {
        if (T::Len)
...
template<class T>
inline DINT TmString<T>::Insert(DINT nIndex, char ch)
{
    if (StrLen() > nIndex)
    {
        T::Expand(nIndex, 1);
...
template<class T>
inline DINT TmString<T>::Insert(DINT nIndex, const char *pstr)
{
...
template<class T>
BOOL TmString<T>::Replace(TmString<T> dst, TmString<T> src, DINT offset)
{
    if (!dst.Len) return 0;
...
template<class T>
void TmString<T>::Mid(TmString &res, DINT first, DINT count)
{
    if (!T::ptr)
        count = 0;
    else if (count < 0)
...
//---------------------------------------------
template<class T>
inline TmString<T> &TmString<T>::operator+=(const char *s)
{
    if (s)
    {
        if (T::Len)
...
inline TmString<T> &TmString<T>::operator=(const char *s)
{
    ...
    if (s && HIWORD(s) == 0)
    {
        ...
        TCHAR *b = (TCHAR * )alloca(1024 * sizeof(TCHAR));
...
//---------------------------------------------
template<class T>
inline BOOL TmString<T>::Cmp(const char *s) const
{
...
//---------------------------------------------
template<class T>
void TmString<T>::Format(const char *pFormat, ...)
{
...
#ifndef _EOS_LINUX_
template <class T>
void TmString<T>::Format(const wchar_t *_pFormat, ...)
{ 
    TmString<T> c;
    c = _pFormat;
...
//---------------------------------------------
template<class T>
inline TmString<T> &TmString<T>::operator=(const wchar_t *s)
{
    ...
    if (s && HIWORD(s) == 0)
    {
        ...
        UINT nID = LOWORD((DWORD)(DWORD_PTR)s);
        HINSTANCE hInst = GetDllResourceInstance();
        TCHAR* b = (TCHAR*)_alloca(1024 * sizeof(TCHAR));
...
template<class T>
bool TmString<T>::_copy(const char *b)
{
    int n;
    bool fOk = false;
    if (b && (n = (int) strlen(b)))
    {
        if (b >= T::ptr && b < T::ptr + T::Len)
        {
            char *s = (char *) _alloca(n + 1);
            memcpy(s, b, n + 1);
            b = (const char *) s;
...
//---------------------------------------------
template<class T>
bool TmString<T>::_copy(const wchar_t *b)
{
    int n;
#ifndef ANDROID_NDK
...
//---------------------------------------------
template<class T>
inline TmString<T> operator+(char ch, TmString<T> &str)
{
    TmString<T> s;
    s.Add() = ch;
    s.Add() = 0;
...
//---------------------------------------------
template<class T>
inline TmString<T> operator+(const char *psz, TmString<T> &str)
{
    TmString<T> s = psz;
    s += str;
    return s;
...
template<class T>
inline TmString<T> &TmString<T>::operator+=(char s)
{
    char ss[2];
    ss[0] = s;
    ss[1] = 0;
...
void WTmString<T>::Format(const wchar_t *pFormat, ...)
{
    ...
    T::ReAlloc((DINT) wcslen(pFormat) + 1024);
    va_list args;
    va_start(args, pFormat);
#ifdef __PLUK_LINUX__
...
template<class T>
void
TmString<T>::Tokenize(const char *_control, MVector<TmString<T> > &arr, bool fAll, bool fClear0)
{
    if (!StrLen()) return;
...
template<class T>
void TmString<T>::Tokenize(const char *control, MArray<int> &arr, bool fAll, bool fClear0)
{
    if (!StrLen()) return;
...
template<class T>
void
WTmString<T>::Tokenize(const wchar_t *control, MVector<WTmString<T> > &arr, bool fAll, bool fClear0)
{
    if (!StrLen()) return;
...
//--------------------------------------
template<class T>
inline WTmString<T> operator+(wchar_t ch, WTmString<T> &str)
{
    WTmString<T> s;
    s += ch;
    s += str;
...
//--------------------------------------
template<class T>
inline WTmString<T> operator+(const wchar_t *psz, WTmString<T> &str)
{
    WTmString<T> s;
    s = psz;
    s += str;
...
//--------------------------------------
template <class T>
inline wchar_t *TmString<T>::AllocSysString()
{
...
#endif
#endif
typedef TmString<MArray<char> > TString;
