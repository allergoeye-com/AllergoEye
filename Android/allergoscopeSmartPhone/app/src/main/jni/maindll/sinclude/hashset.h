...
public:
    ...
    void Build(MArray<MAP_KEY> &c)
    {
        Clear();
        Build(c.GetPtr(), c.GetLen());
    }

    void Build(MAP_KEY *p, int l)
    {
        Clear();
        while (l--) Add(*p++);
    }
    ...
    virtual MListEntry<HKeyNode<MAP_KEY> > *Last()
    { return LIST_ALLOC::Last(); }

    virtual MListEntry<HKeyNode<MAP_KEY> > *First()
...
protected:
    ...
    void GetData(HKeyNode<MAP_KEY> *root, MArray<HKeyNode<MAP_KEY> *> &stack,
                 MArray<HKeyNode<MAP_KEY> *> &res);
...
template<class MAP_KEY, class ARRAY, class LIST_ALLOC>
inline bool MHashSet<MAP_KEY, ARRAY, LIST_ALLOC>::SetRoot(HKeyNode<MAP_KEY> **root, MAP_KEY &key,
                                                          HKeyNode<MAP_KEY> **node)
{
    if (*node)
        *root = *node;
    else
...
//----------------------------------------------
template<class MAP_KEY, class ARRAY, class LIST_ALLOC>
bool MHashSet<MAP_KEY, ARRAY, LIST_ALLOC>::Insert(HKeyNode<MAP_KEY> **root, MAP_KEY &key,
                                                  HKeyNode<MAP_KEY> **node)
{
...
template<class MAP_KEY, class ARRAY, class LIST_ALLOC>
void MHashSet<MAP_KEY, ARRAY, LIST_ALLOC>::GetData(HKeyNode<MAP_KEY> *root,
                                                   MArray<HKeyNode<MAP_KEY> *> &stack,
                                                   MArray<HKeyNode<MAP_KEY> *> &res)
{
    bool fCheck = true;
...
//-------------------------------------------------------
template<class MAP_KEY, class ARRAY, class LIST_ALLOC>
inline MHashSet<MAP_KEY, ARRAY, LIST_ALLOC> &
MHashSet<MAP_KEY, ARRAY, LIST_ALLOC>::operator=(MHashSet<MAP_KEY, ARRAY, LIST_ALLOC> &m)
{
    Clear();
    iMaxSize = m.iMaxSize;
...
class RBHashSet
        : protected MArray<RBKeyNode<MAP_KEY> *>, protected RBSet<MAP_KEY, CMP, LIST_ALLOC> {
public:
    ...
    RBHashSet(int size = 100, int rank = 2)
    {
        iRank = rank;
        iStartSize = size, mDefSize = size;
        RBMALLOCH::SetLen(size);
    ...
    RBKeyNode<MAP_KEY> *Nil()
    { return RBMMAPH::Nil(); }

    virtual ListNotNewEntry<RBKeyNode<MAP_KEY> > *Last()
    { return LIST_ALLOC::Last(); }

    virtual ListNotNewEntry<RBKeyNode<MAP_KEY> > *First()
    { return LIST_ALLOC::First(); }
...
public:
    ...
    class iterator {
    public:
        iterator(RBHashSet &m) : M(m)
        {
            hi = M.MArray<RBKeyNode<MAP_KEY> *>::GetLen();
            it = new RBiterator(m);
    ...
    class iteratorNode {
    public:
        iteratorNode(RBHashSet &m) : M(m)
        {
            it = new RBiteratorNode(m);
...
//-------------------------------------------------------
template<class MAP_KEY, class CMP, class LIST_ALLOC>
RBHashSet<MAP_KEY, CMP, LIST_ALLOC> &
RBHashSet<MAP_KEY, CMP, LIST_ALLOC>::operator=(RBHashSet<MAP_KEY, CMP, LIST_ALLOC> &m)
{
    Clear();
...
{
    return (DWORD) ((ID % (DWORD) MArray<RBKeyNode<MAP_KEY> *>::Len +
                     ID / (DWORD) MArray<RBKeyNode<MAP_KEY> *>::Len) %
                    (DWORD) MArray<RBKeyNode<MAP_KEY> *>::Len);
}
