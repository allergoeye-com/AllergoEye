...
struct Vector3F {
    ...
    void Set(int ix, int iy, int iz = 0)
    {
        x = (float) ix;
        y = (float) iy;
        z = (float) iz;
    ...
    void Set(DINT ix, DINT iy, DINT iz = 0)
    {
        x = (float) ix;
        y = (float) iy;
        z = (float) iz;
    ...
    void Set(size_t ix, size_t iy, size_t iz = 0)
    {
        x = (float) ix;
        y = (float) iy;
        z = (float) iz;
    ...
    bool operator<(const Vector3F &p) const
    {
        if (fabs(x - p.x) > PRECISION_F)
            ...
            return y < p.y;
        else if (fabs(z - p.z) > PRECISION_F)
            return z < p.z;
        return false;
    };
    ...
    Vector3F Int() const
    { return Vector3F((float) floor(x), (float) floor(y), (float) floor(z)); }

    Vector3F Abs() const
    { return Vector3F((float) fabs(x), (float) fabs(y), (float) fabs(z)); }
    ...
    {
        float l = Len();
        return l ? Vector3F(x / l, y / l, z / l) : Vector3F(0, 0, 0);
    }

    void NormalAndLen(Vector3F &n, float &l)
    {
...
//
//------------------------------------
inline Vector3F operator+(const Vector3F &p, const Vector3F &p1)
...
//
//------------------------------------
inline float operator*(const Vector3F &p, const Vector3F &p1)
...
inline Vector3F operator/(const Vector3F &p, const Vector3F &p1)
{
    return Vector3F(fabs(p1.x) > PRECISION_F ? p.x / p1.x : 0.0f,
                    fabs(p1.y) > PRECISION_F ? p.y / p1.y : 0.0f,
                    fabs(p1.z) > PRECISION_F ? p.z / p1.z : 0.0f);
}

//------------------------------------
//
//------------------------------------
inline Vector3F operator+(const float d, const Vector3F &p)
{ return Vector3F(p.x + d, p.y + d, p.z + d); }

//------------------------------------
//
//------------------------------------
inline Vector3F operator-(const Vector3F &p, float d)
{ return Vector3F(p.x - d, p.y - d, p.z - d); }

//------------------------------------
//
//------------------------------------
inline Vector3F operator*(float i, const Vector3F &p)
{ return Vector3F(p.x * i, p.y * i, p.z * i); }

//------------------------------------
//
//------------------------------------
inline Vector3F operator*(const Vector3F &p, float i)
{ return Vector3F(p.x * i, p.y * i, p.z * i); }
...
//
//------------------------------------
inline Vector3F Vector3F::Normal(Vector3F &d)
{
    Vector3F v(0, 0, 0);
    return ::Normal(*this, d, v);
}

//------------------------------------
//
//------------------------------------
inline float Vector3F::Angle(Vector3F &p2)
{ return (float) (acos(Ort() * p2.Ort()) * 180.0 / PI); }
...
struct Vector3FC : public Vector3F {
    ...
    Vector3FC(float ix = 0.0f, float iy = 0.0f, float iz = 0.0f, INT32 icolor = 0) : Vector3F(ix,
                                                                                              iy,
                                                                                              iz)
    { color = icolor; }

    Vector3FC(float s[3]) : Vector3F(s)
    { color = 0; }

    Vector3FC(const Vector3F &s) : Vector3F(s)
    { color = 0; }

    Vector3FC(const Vector3FC &s) : Vector3F(s)
    { color = s.color; }
    ...
    Vector3FC &operator=(const Vector3F &c)
    {
        x = c.x;
        y = c.y;
        z = c.z;
    ...
    Vector3FC &operator=(const Vector3FC &c)
    {
        x = c.x;
        y = c.y;
        z = c.z;
...
inline Vector3F::operator Vector3FC() const
{ return Vector3FC(x, y, z, 0); }
