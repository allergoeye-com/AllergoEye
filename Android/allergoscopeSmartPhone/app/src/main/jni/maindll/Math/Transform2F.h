...
struct Transform2F {
    ...
    Transform2F(const RC &_rDisp, const RC &_rDes, bool prop = true)
    { Init(_rDisp, _rDes, prop); };

    Transform2F()
    {};

    Transform2F(const Transform2F &a)
    { operator=(a); }
...
void Transform2F<T, RC, _TYPE>::Init(const RC &rDisp, const RC &rDes, bool prop)
{
    ...
    if (prop)
        form.m22ScaleY = form.m11ScaleX =
                form.m22ScaleY > form.m11ScaleX ? form.m11ScaleX : form.m22ScaleY;
...
inline void Transform2F<T, RC, _TYPE>::LPtoDP(T *ptr, int len)
{
    while (len--)
    {
        ...
        ptr->x = (_TYPE) (form.m11ScaleX * (float) ptr->x + form.m12ShiftX * (float) ptr->y +
                          form.m13Dx);
