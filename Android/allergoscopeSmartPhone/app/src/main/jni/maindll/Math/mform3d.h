...
struct MFORM3D {
    ...
    MFORM3D(const MFORM3D &m)
    { memcpy(dwM, m.dwM, sizeof(dwM)); }

    void Move(double x, double y, double z);

    bool operator==(MFORM3D &f);

    const MFORM3D &operator=(const MFORM3D &m)
    {
        memcpy(dwM, m.dwM, sizeof(dwM));
        return *this;
    }
    ...
    void Clear0()
    { for (int i = 0; i < 16; ++i) if (fabs(arM[i]) < PRECISION)arM[i] = 0.0; }
    ...
    void
    Frustrum(double left, double top, double right, double bottom, double dbFar, double dbNear);
