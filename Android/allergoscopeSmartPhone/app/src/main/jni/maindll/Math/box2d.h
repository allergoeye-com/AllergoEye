...
struct Box2D {
    ...
    Box2D(const Vector2D &v1, const Vector2D &v2)
    { Set(v1, v2); }

    Box2D(int left, int top, int right, int bottom)
    { Set(Vector2D(left, top), Vector2D(right, bottom)); }

    void Set(const Box2D &b)
    {
        minPoint = b.minPoint;
        maxPoint = b.maxPoint;
    }
    Box2D(const IRect rc)
    {
        minPoint.x = rc.left;
        minPoint.x = rc.top;
        maxPoint.x = rc.right;
        maxPoint.y = rc.bottom;
    ...
    void Set(double x1, double y1, double x2, double y2) { Set(Vector2D(x1, y1), Vector2D(x2, y2)); }
    ...
    double left() const  { return minPoint.x; }
    double top()  const { return minPoint.y; }
    double right()  const { return maxPoint.x; }
    double bottom()  const { return maxPoint.y; }
