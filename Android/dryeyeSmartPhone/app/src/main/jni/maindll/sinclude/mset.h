...
public:
    ...
    RBKeyNode<MAP_KEY> *Root()
    { return pRoot; }

    RBKeyNode<MAP_KEY> *Nil()
    { return &m_Nil; }

    bool Add(const MAP_KEY &key)
    { return Add(key, 0); }

    bool operator[](const MAP_KEY &key)
    { return Find(key); }

    bool Find(const MAP_KEY &key);

    RBKeyNode<MAP_KEY> *Get(const MAP_KEY &key)
    { return Search(pRoot, key); }

    void Delete(const MAP_KEY &key)
    {
    ...
    MAP_KEY Min()
    { return Min(pRoot)->key; }

    MAP_KEY Max()
    { return Max(pRoot)->key; }
    ...
    void GetSortedRBData(MVector<MAP_KEY> &arr)
    { ::GetSortedRBData(arr, *this); }
...
//----------------------------------------------
template<class MAP_KEY, class CMP, class LIST_ALLOC>
inline RBKeyNode<MAP_KEY> *
RBSet<MAP_KEY, CMP, LIST_ALLOC>::Search(RBKeyNode<MAP_KEY> *node, const MAP_KEY &key)
{
...
//---------------------------------------
template<class MAP_KEY, class CMP, class LIST_ALLOC>
inline RBSet<MAP_KEY, CMP, LIST_ALLOC> &
RBSet<MAP_KEY, CMP, LIST_ALLOC>::operator=(RBSet<MAP_KEY, CMP, LIST_ALLOC> &a)
{
    Clear();
...
//----------------------------------------------
template<class MAP_KEY, class CMP, class LIST_ALLOC>
inline void RBSet<MAP_KEY, CMP, LIST_ALLOC>::OR(RBSet<MAP_KEY, CMP, LIST_ALLOC> &in,
                                                RBSet<MAP_KEY, CMP, LIST_ALLOC> &out)
{
    RBSet *a, *b;
...
//----------------------------------------------
template<class MAP_KEY, class CMP, class LIST_ALLOC>
bool RBSet<MAP_KEY, CMP, LIST_ALLOC>::AND(RBSet<MAP_KEY, CMP, LIST_ALLOC> &in,
                                          RBSet<MAP_KEY, CMP, LIST_ALLOC> &out)
{
    RBSet *a, *b;
    out.Clear();
