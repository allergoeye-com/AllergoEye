...
public:
    ...
    void Detach()
    {
        MArray<USHORT>::Detach();
        iMin = iMax = 0;
    }

    MBoolArray(int iMin = 0, int iMax = 1024);

    MBoolArray(MBoolArray &s)
    { *this = s; }
    ...
    MBoolArray operator|(MBoolArray &s2)
    { return MBoolArray(*this) |= s2; }
...
public:
    ...
    MBoolDim operator|(MBoolDim &s2)
    { return MBoolDim(*this) |= s2; }
...
inline bool MBoolArray::operator[](int n)
{
    ...
    return n <= iMax && n >= iMin ? (*(ptr + ((n - iMin) >> 4)) & (1 << ((n - iMin) & 0x0f))) != 0
                                  : false;
}
