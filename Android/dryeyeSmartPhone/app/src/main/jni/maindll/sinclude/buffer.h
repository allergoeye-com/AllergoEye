...
inline DWORD SetLastError(DWORD dw)
{
    if (!dw) errno = 0;
    return 0;
}
...
#define m_Alloc  (HeapAlloc)
    ...
    void SetAddedRealSize(int dw)
    { defAdded = dw > 0 ? (int) (dw / sizeof(T)) : dw; }
...
protected:
    ...
    void CopyTo(T *buff)
    { if (Len) MemCpy(buff, ptr, Len); }

    void Detach(HANDLE _hHeap = GetProcessHeap())
    {
        ptr = 0;
        RealSize = Len = 0;
        hHeap = _hHeap;
...
//	BoolWarning (len >= 0);
    ...
    DINT add = len;
    if (defAdded > -1)
        add = RealSize > (DINT) defAdded ? defAdded : (len > (DINT) defAdded ? (DINT) defAdded
                                                                             : len);
...
inline BOOL MMalloc<T>::ReAlloc(DINT len)
{
    ...
    DINT add = len;
    if (defAdded > -1)
        add = RealSize > (DINT) defAdded ? (DINT) defAdded : (len > (DINT) defAdded
                                                              ? (DINT) defAdded : len);
...
public :
    ...
    char IsValid()
    { return MMalloc<T>::ptr != 0 || !MMalloc<T>::Len; }

    DINT GetLen() const
    { return MMalloc<T>::Len; }

    BOOL Compact()
    { return MMalloc<T>::Compact(); }

    DINT GetRealSize()
    { return MMalloc<T>::RealSize; }

    void SetLen(DINT l)
    {
        if (MMalloc<T>::Len != l)
        { if (MMalloc<T>::RealSize >= l) MMalloc<T>::Len = l; else MMalloc<T>::ReAlloc(l); }
    }
    ...
    T *GetPtr() const
    { return MMalloc<T>::ptr; }

    void Remove(DINT pos, DINT num);

    void MoveTo(T **p, DINT &len)
    {
        *p = MMalloc<T>::ptr;
        len = MMalloc<T>::Len;
        MMalloc<T>::RealSize = 0;
        MMalloc<T>::Len = 0;
        MMalloc<T>::ptr = 0L;
    }

    void Init(T *p, DINT len)
    {
        MMalloc<T>::ptr = p;
        MMalloc<T>::Len = len;
        MMalloc<T>::RealSize = MMalloc<T>::Len;
    }
    ...
    BOOL Append(MBuffer &src)
    {
        DINT i = MMalloc<T>::Len;
        BOOL ret = MMalloc<T>::ReAlloc(MMalloc<T>::Len + src.Len);
        if (ret) MMalloc<T>::MemCpy(MMalloc<T>::ptr + i, src.ptr, src.Len);
        return ret;
    }
    ...
    BOOL Expand(DINT pos, DINT num)
    { return MMalloc<T>::ExpandBuff(pos, num); }

    T &operator[](DINT i) const
    { return MMalloc<T>::ptr[i]; }
...
inline void MBuffer<T>::Remove(DINT pos, DINT num)
{
    ...
    {
        MMalloc<T>::MemCpy(MMalloc<T>::ptr + pos, MMalloc<T>::ptr + pos + num,
                           MMalloc<T>::Len - pos - num);
...
public:
    ...
    {
        DINT i = MMalloc<T>::Len;
        BOOL ret = ReAlloc(i + 1);
        if (ret) MMalloc<T>::ptr[i] = t;
        return ret;
    }

    void Swap(__Vector &m)
    { MMalloc<T>::Swap(m); }
    ...
    {
        DINT i = MMalloc<T>::Len;
        BOOL ret = ReAlloc(MMalloc<T>::Len + num);
        if (ret) MemCpy(MMalloc<T>::ptr + i, src, num);
        return ret;
    }

    BOOL Append(__Vector &src)
    {
        DINT i = MMalloc<T>::Len;
        BOOL ret = ReAlloc(MMalloc<T>::Len + src.Len);
        if (ret) MemCpy(MMalloc<T>::ptr + i, src.ptr, src.Len);
        return ret;
    }
    ...
    __Vector &operator=(const __Vector &c)
    {
        SetLen(c.Len);
        Copy(c.ptr, c.Len);
        return *this;
...
public :
    ...
    void Attach(T *src, DINT len, bool fKill = false)
    {
        K::Free();
        K::ptr = src;
        K::RealSize = K::Len = len;
        if (!fKill) K::hHeap = 0;
    }
    ...
    BOOL Replace(DINT pos, _ARRAY &a)
    { return Replace(pos, a.ptr, a.Len); }

    BOOL Replace(DINT pos, const T *src, DINT num);

    BOOL Insert(DINT pos, const T *src, DINT num);

    BOOL Insert(DINT pos, _ARRAY &src)
    { return Insert(pos, src.ptr, src.Len); }

    BOOL Insert(DINT pos, const T &src)
    { return K::Insert(pos, src); }
...
private:
    void
    helpSort(DINT *item, DINT left, DINT right, int (__cdecl *func)(const void *, const void *));
...
inline void _ARRAY<T, K>::InsertBHeap(const T &t, int (__cdecl *func)(const void *, const void *))
{
    ...
    if (K::RealSize > K::Len + 1)
        ++K::Len;
    else if (!K::ReAlloc(K::Len + 1)) return;
...
//------------------------------------------
template<class T, class K>
void _ARRAY<T, K>::helpSort(DINT *item, DINT left, DINT right,
                            int (__cdecl *func)(const void *, const void *))
{
    DINT i, j, y;
    i = left;
    j = right;
...
template<class T, class K>
void _ARRAY<T, K>::helpSort(int *item, int left, int right,
                            int (__cdecl *func)(const void *, const void *))
{
    int i, j, y;
    i = left;
...
template<class T, class K>
inline DINT
_ARRAY<T, K>::Find(const T &t, DINT offset, BOOL (__cdecl *func)(const void *, const void *))
{
    if (offset >= K::Len)
        return -1;
...
//	void FreeInRealloc(bool fFree) { 	_ARRAY<T, MBuffer<T> >::FreeInRealloc(fFree); }
    ...
    MArray(DINT i, const T &t) : _ARRAY<T, MBuffer<T> >(i)
    { _ARRAY<T, MBuffer<T> >::Set(t); }
    ...
    {
        _ARRAY<T, MBuffer<T> >::operator=(b);
        return *this;
    }

    BOOL operator==(const MArray &b)
    { return _ARRAY<T, MBuffer<T> >::operator==(b); }
    ...
    {
        MArray tail;
        _ARRAY<T, MBuffer<T> >::Tail(tail, i);
        return tail;
    }
    ...
    {
        MArray left;
        _ARRAY<T, MBuffer<T> >::Left(left, i);
        return left;
    }
    ...
    {
        MArray right;
        _ARRAY<T, MBuffer<T> >::Right(right, i);
        return right;
    }

    MArray Mid(DINT first, DINT count)
    {
        MArray mid;
        _ARRAY<T, MBuffer<T> >::Mid(mid, first, count);
        return mid;
    }
...
public:
    ...
    MArrayByte &operator=(const MArrayByte &b)
    {
        _ARRAY<BYTE, MBuffer<BYTE> >::operator=(b);
        return *this;
    }

    BOOL operator==(const MArrayByte &b)
    { return _ARRAY<BYTE, MBuffer<BYTE> >::operator==(b); }
    ...
    MArrayByte &operator+=(BYTE &b)
    {
        Add(b);
        return *this;
    }

    MArrayByte Tail(DINT i)
    {
        MArrayByte tail;
        _ARRAY<BYTE, MBuffer<BYTE> >::Tail(tail, i);
        return tail;
    }

    MArrayByte Left(DINT i)
    {
        MArrayByte left;
        _ARRAY<BYTE, MBuffer<BYTE> >::Left(left, i);
        return left;
    }

    MArrayByte Right(DINT i)
    {
        MArrayByte right;
        _ARRAY<BYTE, MBuffer<BYTE> >::Right(right, i);
        return right;
    }

    MArrayByte Mid(DINT first, DINT count)
    {
        MArrayByte mid;
        _ARRAY<BYTE, MBuffer<BYTE> >::Mid(mid, first, count);
        return mid;
    }
...
public:
    ...
    MVector(DINT i, const T &t) : _ARRAY<T, __Vector<T> >(i)
    { Set(t); }
    ...
    BOOL operator==(const MVector &b)
    { return _ARRAY<T, __Vector<T> >::operator==(b); }
    ...
    {
        _ARRAY<T, __Vector<T> >::operator=(b);
        return *this;
    }
    ...
    MVector &operator+=(const T &b)
    {
        _ARRAY<T, __Vector<T> >::Add(b);
        return *this;
    }
    ...
    {
        MVector tail;
        _ARRAY<T, __Vector<T> >::Tail(tail, i);
        return tail;
    }
    ...
    {
        MVector left;
        _ARRAY<T, __Vector<T> >::Left(left, i);
        return left;
    }

    MVector Right(DINT i)
    {
        MVector right;
        _ARRAY<T, __Vector<T> >::Right(right, i);
        return right;
    }

    MVector Mid(DINT first, DINT count)
    {
        MVector mid;
        _ARRAY<T, __Vector<T> >::Mid(mid, first, count);
        return mid;
    }
};
