...
public:
    MMap(int size = 32) : RBHashMap<MAP_VALUE, MAP_KEY, CMP, LIST_ALLOC>(size)
    { RBHashMap<MAP_VALUE, MAP_KEY, CMP, LIST_ALLOC>::KeyFromCompareFunc(true); };
    ...
    MMap &operator=(MMap &a)
    {
        RBHashMap<MAP_VALUE, MAP_KEY, CMP, LIST_ALLOC>::operator=(a);
        return *this;
    }
};
