...
public:
    ArHashMap(int _iRank = 2, int max_alloc_block = 1024, int min_alloc_block = 4,
              int _iStartSize = 128)
    {
...
public:
    RBData<MAP_VALUE, MAP_KEY> *VData(int &len)
    {
        len = Data.GetLen() - 1;
        return bData;
    }

    ArHashTreeNode *VTree(int &len)
    {
        len = Tree.GetLen() - 1;
        return bTree;
    }
...
/*******************from serialize class SwapFile only *************************************************/
    BOOL MoveToRoot(DINT *buffer, DINT len)
    {
        ARRAY::ptr = buffer;
        ARRAY::Len = len;
        return true;
...
template<class MAP_VALUE, class MAP_KEY, class ARRAY>
inline bool
ArHashMap<MAP_VALUE, MAP_KEY, ARRAY>::SetRoot(DINT *root, MAP_KEY &key, MAP_VALUE *value,
                                              DINT *node)
{
    if (*node != -1)
        *root = *node;
...
template<class MAP_VALUE, class MAP_KEY, class ARRAY>
inline bool
ArHashMap<MAP_VALUE, MAP_KEY, ARRAY>::AllocNode(MAP_KEY &key, MAP_VALUE *value, DINT *node)
{
    if (*node == -1)
    {
...
template<class MAP_VALUE, class MAP_KEY, class ARRAY>
bool
ArHashMap<MAP_VALUE, MAP_KEY, ARRAY>::Insert(DINT *root, MAP_KEY &key, MAP_VALUE *value, DINT *node)
{
...
bool ArHashMap<MAP_VALUE, MAP_KEY, ARRAY>::Insert(DINT *root, DINT node)
{
    ...
    while (temp != -1)
    {
        ...
        if ((n = (bData[node].key < bData[temp].key)))
            temp = bTree[temp].left;
        else if (bData[node].key > bData[temp].key)
            temp = bTree[temp].right;
        else
            return false;
    }
...
template<class MAP_VALUE, class MAP_KEY, class ARRAY>
void
ArHashMap<MAP_VALUE, MAP_KEY, ARRAY>::GetData(DINT root, MArray<DINT> &stack, MArray<DINT> &res)
{
    bool fCheck = true;
...
//---------------------------------------------------------------------------
template<class MAP_VALUE, class MAP_KEY, class ARRAY>
inline ArHashMap<MAP_VALUE, MAP_KEY, ARRAY> &
ArHashMap<MAP_VALUE, MAP_KEY, ARRAY>::operator=(ArHashMap<MAP_VALUE, MAP_KEY, ARRAY> &m)
{
    Clear();
    iMaxSize = m.iMaxSize;
