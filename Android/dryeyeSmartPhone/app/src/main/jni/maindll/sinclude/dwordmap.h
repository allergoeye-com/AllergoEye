...
struct RBDWordNode {
    ...
    RBDWordNode &operator=(RBDWordNode &r)
    {
        color = r.color;
        parent = r.parent;
        left = r.left;
    ...
    void
    Init(RBDWordNode *_left, RBDWordNode *_right, RBDWordNode *_parent, bool _color, DWORD &_key,
         MAP_VALUE &_value)
    {
        left = _left;
        right = _right;
...
public:
    ...
    void Delete(DWORD key)
    {
        RBDWordNode<MAP_VALUE> *p = Search(pRoot, key);
        if (p != &m_Nil) DeleteNode(p);
    }

    MAP_VALUE &Value(DWORD key)
    { return Search(pRoot, key)->value; }
    ...
    RBDWordNode<MAP_VALUE> *Root()
    { return pRoot; }

    RBDWordNode<MAP_VALUE> *Nil()
    { return &m_Nil; }

    const RBDWordNode<MAP_VALUE> *Max()
    {
    ...
    const RBDWordNode<MAP_VALUE> *Min()
    {
        RBDWordNode<MAP_VALUE> *node = pRoot;
...
protected:
    ...
    void Copy(RBMapDword &map, RBDWordNode<MAP_VALUE> *node, RBDWordNode<MAP_VALUE> *parent,
              RBDWordNode<MAP_VALUE> **self)
    {
        if (node == &map.m_Nil)
            *self = &m_Nil;
        else
...
//----------------------------------------------
template<class MAP_VALUE, class _ALLOCATOR_>
inline RBDWordNode<MAP_VALUE> *
RBMapDword<MAP_VALUE, _ALLOCATOR_>::Search(RBDWordNode<MAP_VALUE> *node, DWORD &key)
{
...
//----------------------------------------------
template<class MAP_VALUE, class _ALLOCATOR_>
bool RBMapDword<MAP_VALUE, _ALLOCATOR_>::Insert(DWORD &key, MAP_VALUE *value,
                                                RBDWordNode<MAP_VALUE> **node)
{
...
//---------------------------------------
template<class MAP_VALUE, class _ALLOCATOR_>
RBMapDword<MAP_VALUE, _ALLOCATOR_> &
RBMapDword<MAP_VALUE, _ALLOCATOR_>::operator=(RBMapDword<MAP_VALUE, _ALLOCATOR_> &a)
{
    Clear();
    if (a.pRoot != &a.m_Nil)
