...
public:
    ...
    ConstSet(const ConstSet &a)
    { *this = a; }

    ConstSet &operator=(const ConstSet &a)
    {
        tree = a.tree;
        iMaxRank = a.iMaxRank;
        nodeLen = tree.GetLen();
        node_ptr = tree.GetPtr();
        MAP_ARRAY::operator=(a);
        return *this;
    }

    DINT MaxRank(DINT i = 0)
    {
        if (i > 0) iMaxRank = i;
        return iMaxRank;
    }
    ...
    MAP_KEY *DataPtr()
    { return MAP_ARRAY::ptr; }

    ConstMapNode *TreePtr()
    { return tree.GetPtr(); }
...
protected:
    bool FindFirst(MAP_KEY &res, ConstMapNode &node, MArray<DINT> &stack,
                   ConstSet<MAP_KEY, MAP_ARRAY> *check, MAP_KEY *pMin, MAP_KEY *pMax);
...
protected:
    ...
    void GetSortedRBData(MAP_ARRAY &arr, ConstMapNode &node, MArray<DINT> &stack, DINT op = OP_NONE,
                         ConstSet *check = 0, MAP_KEY *pMin = 0, MAP_KEY *pMax = 0);

    void Build(MAP_ARRAY &sortedArray, DINT y, MArray<DINT> &rank,
...
bool
ConstSet<MAP_KEY, MAP_ARRAY>::FindFirstIntersect(ConstSet<MAP_KEY, MAP_ARRAY> &in, MAP_KEY &out)
{
...
//-----------------------------------------------------------
template<class MAP_KEY, class MAP_ARRAY>
inline void ConstSet<MAP_KEY, MAP_ARRAY>::Build(MAP_ARRAY &sortedArray, DINT y, MArray<DINT> &rank,
                                                MArray<ConstMapStack<MAP_KEY> > &stack)
{
    DINT prevLen = MAP_ARRAY::Len;
    ...
    n1 = 0;
    SortStaticMap(MAP_ARRAY::ptr, rank.GetPtr(), n1, sortedArray.GetPtr(), sortedArray.GetLen(), y,
                  stack);
    MAP_ARRAY::ptr = p;
...
{
    DINT i = 0;
    while (i < nodeLen && (*(MAP_ARRAY::ptr + (node_ptr + i)->key) > key ||
                           *(MAP_ARRAY::ptr + (node_ptr + i)->key_max) < key))
...
//-----------------------------------------------------------
template<class MAP_KEY, class MAP_ARRAY>
void ConstSet<MAP_KEY, MAP_ARRAY>::GetSortedRBData(MAP_ARRAY &arr, ConstMapNode &node,
                                                   MArray<DINT> &stack, DINT op,
                                                   ConstSet<MAP_KEY, MAP_ARRAY> *check,
                                                   MAP_KEY *pMin, MAP_KEY *pMax)
{
...
//-----------------------------------------------------------
template<class MAP_KEY, class MAP_ARRAY>
bool ConstSet<MAP_KEY, MAP_ARRAY>::FindFirst(MAP_KEY &res, ConstMapNode &node, MArray<DINT> &stack,
                                             ConstSet<MAP_KEY, MAP_ARRAY> *check, MAP_KEY *pMin,
                                             MAP_KEY *pMax)
{
    MAP_KEY *root = MAP_ARRAY::ptr + node.pos;
