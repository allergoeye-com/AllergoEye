...
//
//-----------------------------------------------------
int RuningPoygon::IsClipXRay(Segment2D &p, Vector2D n[2])
{
    Vector2D pt;
    double t;
...
bool RuningPoygon::ClipLines(int _iIndex, DecPos pos[2], Vector2D n[2], bool fInStack)
{
    ...
    while (node)
    {
        ...
        if ((fOrg && fDest) ||
            (fOrg && (node->value.dest == s.dest || node->value.dest == s.org)) ||
            (fDest && (node->value.org == s.dest || node->value.org == s.org)) ||
            node->value.org == node->value.dest)
        {
