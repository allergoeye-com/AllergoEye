...
#pragma pack(1)
	...
	struct FunCall {
		...
		int num_param;
		Param param[1];
		void Init(LPCSTR call) { strcpy_s(name, 8, call); len = sizeof(FunCall); num_param = 0; memset(param, 0, sizeof(Param)); }
...
public:
	struct Msg {
		...
		WPARAM w;
		LPARAM l;
		Msg(UINT _m = 0, WPARAM _w = 0, LPARAM _l = 0) { Init(_m, _w, _l); }
		void Init(UINT _m, WPARAM _w, LPARAM _l) { m = _m, l = _l, w = _w; }
	};
	struct PacketRead {
		Query cmd;
		BYTE* pResult;
...
#ifdef WIN32
#else
	virtual LRESULT SendMessage(UINT Msg, WPARAM wParam = 0, LPARAM lParam = 0, int timeout = 120000) = 0;
