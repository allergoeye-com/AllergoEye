...
{
public:
    MFrustrum() { iNum = 0; planes = 0; }
	MFrustrum(MFORM3D &trans, MFORM3D &prj) { iNum = 0; planes = 0; Init(trans, prj); }
	MFrustrum(const MFrustrum &p) { planes = new Plane3D[p.iNum]; iNum = p.iNum; memcpy(planes, p.planes, sizeof(Plane3D) * iNum); }
	~MFrustrum() { Reset(); }
	bool IsValid() { return planes != 0; }
	void Reset() { if (planes) delete [] planes; iNum = 0; planes = 0; }
	void Init(MFORM3D &trans, MFORM3D &prj) { MFORM3D m = prj * trans; Init(m); }

	MFrustrum &operator = (const MFrustrum &p) { Reset(); planes = new Plane3D[p.iNum]; iNum = p.iNum; memcpy(planes, p.planes, sizeof(Plane3D) * iNum); return *this; }
	void Init(MFORM3D &m);
	void Init(Box3D &box);
...
inline void MFrustrum::Init(MFORM3D &m)
{
	...
	InitPlane(planes[Plane3D::PL_FRONT], Vector3D(m.arM[3] + m.arM[2], m.arM[7] + m.arM[6], m.arM[11] + m.arM[10]), m.arM[15] + m.arM[14]);
	InitPlane(planes[Plane3D::PL_FAR], Vector3D(m.arM[3] - m.arM[2], m.arM[7] - m.arM[6], m.arM[11] - m.arM[10]), m.arM[15] - m.arM[14]);
	InitPlane(planes[Plane3D::PL_LEFT], Vector3D(m.arM[3] + m.arM[0] , m.arM[7] + m.arM[4], m.arM[11] + m.arM[8]), m.arM[15] + m.arM[12]);
	InitPlane(planes[Plane3D::PL_RIGHT], Vector3D(m.arM[3] - m.arM[0], m.arM[7] - m.arM[4], m.arM[11] - m.arM[8]), m.arM[15] - m.arM[12]);
	InitPlane(planes[Plane3D::PL_TOP], Vector3D(m.arM[3] - m.arM[1], m.arM[7] - m.arM[5], m.arM[11] - m.arM[9]), m.arM[15] - m.arM[13]);
	InitPlane(planes[Plane3D::PL_BOTTOM], Vector3D(m.arM[3] + m.arM[1], m.arM[7] + m.arM[5], m.arM[11] + m.arM[9]), m.arM[15] + m.arM[13]);
...
inline void MFrustrum::Init(Box3D &box)
{
	...
	Vector3D v = box.CenterPoint();
	   ...
	   {
		 if (planes[i].n.x * v.x + planes[i].n.y * v.y + planes[i].n.z * v.z + planes[i].dist < -PRECISION)
			planes[i].Flip();
		}
...
{
	   for(int i = 0; i < iNum; ++i)
		  if(planes[i].n.x * v.x + planes[i].n.y * v.y + planes[i].n.z * v.z + planes[i].dist < -PRECISION_F)
			 return false;
	   return true;
}
...
inline bool MFrustrum::ClipPoint(Vector3D &v1, Vector3D &v2)
{
	...
	for(int i = 0; i < iNum; ++i)
	{
		numer = -(planes[i].n.x * v1.x + planes[i].n.y * v1.y + planes[i].n.z * v1.z + planes[i].dist);
		if(numer > PRECISION)
		{
			dir = v2 - v1;	
			d = dir * planes[i].n;
...
inline bool MFrustrum::IsCross(Box3D &box)
{
	for (int i = 0; i < iNum; ++i)
	{
		if (planes[i].n.x * (planes[i].mask & Vector3D::YZ ? box.minPoint.x : box.maxPoint.x) +
			...
			planes[i].n.z * (planes[i].mask & Vector3D::XY ? box.minPoint.z : box.maxPoint.z) < -PRECISION)
			return false;
	}
	return true;
