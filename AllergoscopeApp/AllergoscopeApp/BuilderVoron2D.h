...
EXPORTNIX class BuilderVoron2D 
{
public:
	BuilderVoron2D() {}
	~BuilderVoron2D() {}
...
//	void Build(MArray<Vector2F> &in, Box2D &rc, GraphTriangle &tri);
	void Build(MArray<Vector2F> &pos, int X, int Y, MArray<GNode> &adj) { MArray<Vector2F> minus; Build(pos, minus, X, Y, adj); }
	void BuildGabriel(MArray<Vector2F> &pos, int X, int Y, MArray<GNode> &adj) { MArray<Vector2F> minus; BuildGabriel(pos, minus, X, Y, adj); }
	void BuildGabriel(MArray<Vector2D> &pos, int X, int Y, MArray<GNode> &adj);

	void BuildGabriel(MArray<Vector2F> &pos, MArray<Vector2F> &minus, int X, int Y, MArray<GNode> &adj);
	void Build(MArray<Vector2F> &pos, MArray<Vector2F> &minus, int X, int Y, MArray<GNode> &adj);

	protected:
	void Build(MArray<Vector2F> &vpt, MArray<Vector2F> &minus, Box2D &rc, MVector<RuningPoygon> &e, MVector<AllocatorLine > &pAlloc);
	void Build(RuningPoygon *pe, TListArray<DINT> &sn, TListArray<DINT> &run);
	void Build(MArray<Vector2D> &m_Data, Box2D &rc, MVector<RuningPoygon> &e, AllocatorLine &pAlloc);
	void Build2(MArray<Vector2D> &m_Data, Box2D &rc, MVector<RuningPoygon> &e, MVector<AllocatorLine > &pAlloc);
