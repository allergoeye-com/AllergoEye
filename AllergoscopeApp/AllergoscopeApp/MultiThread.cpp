...
bool MultiThreadS::Create(int n)
{
    ...
    if (!CreateSemaphore(&semphIn) || !CreateSemaphore(&semphOut) || !CreateSemaphore(&hNotifyOut) ||
    ...
    for (int j = 0; j < num; ++j)
    {
        ...
        int r = pthread_create(&startup[j]->hThread, &startup[j]->dwThreadID, ThreadFunc, (void *)startup[j]);
#endif
        if (r)
        {
            OnError();
