...
struct Vector3D {
	...
	Vector3D &operator = (const POINTL &p) { x = p.x; y = p.y; z = 0; return *this; }
	Vector3D &operator = (const POINT &p) { x = p.x; y = p.y; z = 0; return *this;}
	Vector3D &operator = (const Vector3F &p) { x = p.x; y = p.y; z = p.z; return *this;}
	Vector3D &operator += (const Vector3D &p) { x += p.x, y += p.y; z += p.z; return *this; } 
	Vector3D &operator -= (const Vector3D &p) { x -= p.x, y -= p.y; z -= p.z;  return *this; }
	Vector3D operator - () const { return Vector3D (-x, -y, -z); }	
	Vector3D &operator *= (double i) { x *= i, y *= i; z *= i; return *this; }
	Vector3D &operator -= (double i) { x -= i, y -= i; z -= i; return *this; }
	Vector3D &operator += (double i) { x += i, y += i; z += i; return *this; }
	Vector3D &operator = (double i) { x = i, y = i; z = i; return *this; }
	double operator *= (const Vector3D &p)  { return p.x * x + p.y * y + p.z * z; } 
	double Cos(const Vector3D &p) const;
	operator Vector3F () const { return Vector3F((float)x, (float)y, (float)z); }
	operator Vector2D () const;
	Vector3D &operator /= (double i) { x /= i, y /= i; z /= i; return *this; }
	double &operator [] (int i) { return i < 3 ? axis[i] : z; }
	double operator [] (int i) const { return i < 3 ? axis[i] : z; }
	BOOL IsEqual(const Vector3D &p, double iPRECISION) const {return fabs(x - p.x) <= iPRECISION && fabs(y - p.y) <= iPRECISION && fabs(z - p.z) <= iPRECISION; }
...
//	BOOL operator != (const Vector3D &p)  const { return !operator == (p); }
	BOOL operator == (const Vector3D &b) const { return x == b.x && y == b.y && z == b.z;}
	BOOL operator != (const Vector3D &b) const { return x != b.x || y != b.y || z != b.z;}
	BOOL operator <= (const Vector3D &p) const {  return operator ==(p) || operator < (p); }
	BOOL operator >= (const Vector3D &p) const {  return operator ==(p) || operator > (p); }
...
*/
	...
	double LenQ()const { return x*x + y*y + z*z; }
	double Sum() { return x + y + z; }
	Vector3D Int () const { return Vector3D(int(x), int(y), int(z)); }
	Vector3D Abs () const { return Vector3D(fabs(x), fabs(y), fabs(z)); }
	
	Vector3D Q () const { return Vector3D(x*x, y*y, z*z); }
	double Max() const { return max(max(x, y), z); }
	double Modul() const { return Len(); }
	Vector3D Ort() { double l = Len(); return l ? Vector3D(x/l, y/l, z/l) : Vector3D(0, 0, 0); }
	void NormalAndLen(Vector3D &n, double &l) { l = sqrt (x*x + y*y + z*z); n.x = x / l; n.y = y / l; n.z = z / l; }

	Vector3D &Normalize() { double l = sqrt (x*x + y*y + z*z); if (l > PRECISION) { x /= l; y /= l; z /= l; } return *this; }
...
*******************************/
	...
	int	Orient () //относительно полуплоскостей 
	{
		if (x > PRECISION)
		{
			if (y > PRECISION)
