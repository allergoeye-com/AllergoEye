...
struct Allocator : public MArray<T*> {
	...
	Allocator() 
	{
		...
		SizeOfT = sizeof(T); 
		MArray<T*>::SetAddedRealSize(0x2000000); count = 0; 
	};
	~Allocator();
	...
	BOOL private_alloc();
	void Clear() { Reset(); MArray<T*>::Clear(); }
	bool IsFree();
	int NumBytes();
...
struct MNew : Allocator<T> {
	MNew() {};
	~MNew() {};
	T *Alloc(bool f = 0) { T *p = Allocator<T>::Alloc(false); return new (p) T(); }
	void Free(T *p) { p->~T(); Allocator<T>::Free(p); }
	T *Alloc(T &t) { T *p = Allocator<T>::Alloc(false); return new (p) T(t); }
