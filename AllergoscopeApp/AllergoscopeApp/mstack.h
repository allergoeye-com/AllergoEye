...
public :
	MLifo() { ARR::SetAddedRealSize(-1); };
	~MLifo() {};
	MLifo(MLifo &l): ARR(l) { };
	void Push(const T &t) { ARR::Add(t); } 
	T Pop()  {  return ARR::Len ? ARR::ptr[--ARR::Len] : m_Nil; } 
	T Pop(int i)  { if (i < ARR::Len)  { T tmp = ARR::ptr[i]; ARR::Remove(i, 1); return tmp; } return  m_Nil; }
	bool IsEmpty() const { return ARR::Len == 0; }
	T Last() {  return ARR::Len ? ARR::ptr[ARR::Len - 1] : m_Nil; }
...
{ 
	...
	ItemMFifo *next;
	ItemMFifo() { next = 0; }
	ItemMFifo *Init(const T &x, ItemMFifo *t) { value = x; next = t; return this; }
};

template <class T, class ALLOC = Allocator<ItemMFifo<T> > >
class MFifo  {
public:
	MFifo (bool fNewDelele = true) { m_fNewDelele = fNewDelele;m_iNum = 0; first = last = 0; }
	MFifo(const MFifo &f) { *this = f; } 
	~MFifo () {}
	...
	void Clear() {
 	...
 	ItemMFifo<T> *GetPtr() { return first; }
 	void Push(const T &t) 	
	{ 
...
public:
	...
	{ 
		...
		ItemMFifo<T> *t;
	public:
		iterator(MFifo &g):	stk(g) { t = (ItemMFifo<T> *)0xFFFFFFFF; }
		T *begin()
		{ t = stk.first; return t ? &t->value : 0; }
		T *next()
		{ if (t == (ItemMFifo<T> *)0xFFFFFFFF) return begin(); if (t) t = t->next; return t ? &t->value : 0; }
		bool end()
		{ return t == 0; }
	};
...
struct ItemMDeq:public __DecPos
{
	...
	ItemMDeq *next;
	ItemMDeq *prev;
	ItemMDeq() { prev = next = 0; }
	ItemMDeq *InitF(const T &x, ItemMDeq *t) { prev = 0; value = x; next = t; if (t) t->prev = this; return this; }
	ItemMDeq *InitL(const T &x, ItemMDeq *t) { next = 0; value = x; prev = t; if (t) t->next = this; return this; }
	ItemMDeq *InitF(ItemMDeq *t) { prev = 0;  next = t; if (t) t->prev = this; return this; }
	ItemMDeq *InitL(ItemMDeq *t) { next = 0;  prev = t; if (t) t->next = this; return this; }
	ItemMDeq(ItemMDeq *a)
	{
		 prev = next = 0;
		if (a)
...
class MDeq {
	struct Position {
		...
		void Set(DecPos _pos,  DINT _index) { pos = _pos; index = _index; }
	};
public:

	MDeq (bool fNewDelele = true) { m_fNewDelele = fNewDelele; fKill = true; m_iNum = 0; m_Al = 0; memset(&m_Nil, 0, sizeof(T)); last = first = 0; }
	MDeq(const MDeq &f) { memset(&m_Nil, 0, sizeof(T)); fKill = true; m_iNum = 0; m_Al = 0; last = first = 0; *this = f; }
	virtual ~MDeq () { Clear(); DeleteAlocator(); }
	void CreateNew(bool fNewDelele) { m_fNewDelele = fNewDelele; }
	void DeleteAlocator() { if (m_Al && fKill) delete m_Al; m_Al = 0; fKill = true; }
	void SetAlocator(ALLOC *a, bool _fKill) {ALLOC *tmp = m_Al; 
		if (!a) { Clear(); DeleteAlocator(); }
		m_Al = a; 
		if (tmp)
		{
	...
	//----------------------------------------------------------------
	ItemMDeq<T> *GetPtr() { return first; }
	//----------------------------------------------------------------
	DecPos GetFirstPos() const { return first; }
	//----------------------------------------------------------------
	DecPos GetLastPos() const { return last; }
	//----------------------------------------------------------------
	DecPos GetPos(int i)
	{
	...
	int IsEmpty() { return !m_iNum || first == 0; }
	T m_Nil;
protected:
...
public:
	...
	{ 
		...
		ItemMDeq<T> *t;
	public:
		iterator(MDeq &g, bool _direct = true):	stk(g), direct(_direct) { t = (ItemMDeq<T> *)-1; }
		T *begin()
		{ t = direct ? stk.first : stk.last; return t ? &t->value : 0; }
		T *next()
		{ if (t == (ItemMDeq<T> *)-1) return begin(); if (t) t = direct ? t->next : t->prev; return t ? &t->value : 0; }
		bool end()
		{ return t == 0; }
	};
	friend class iterator;
...
class RandomStack : public ARR {
	public:
	RandomStack(int Size = 0):ARR(Size) { srand(time(0)); ARR::SetAddedRealSize(-1); N = 0; }
	~RandomStack() {}
	RandomStack(RandomStack &s) { *this = s; }
	RandomStack &operator = (RandomStack &s) { Copy(s); N = s.N; return *this; }
	int IsEmpty() { return N == 0; }
	void Push(T t) { if (ARR::Len > N) ARR::ptr[N++] = t; else { ARR::Add(t); ARR::Len = ARR::RealSize - 1; } }
	T Last() { return ARR::ptr[N]; }
	T First() { return *ARR::ptr; }
	T Pop()
	{ 
